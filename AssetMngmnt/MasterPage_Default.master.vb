...
Partial Class MasterPage
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) 'Handles Me.Load
        ...
        If IsPostBack = False Then
            objFunction.RegAppHit(Session("Companycode"), Session("Empcode"), "ops", Request.UserHostAddress)
            Menu1.Items.Clear()
            PopulateMenu()
        End If
    ...
    Private Function GetMenuData() As DataTable
        ...
        Try
            ...
            Dim strSQL As String = "select b.description,page_name=REPLACE(REPLACE(REPLACE(ISNULL(REPLACE(REPLACE(REPLACE(ISNULL(c.page_name,''), '~', ''),'/', ''), '\', ''), ''),'-','/'),'*','~'),'$','/'),parent_menu,seq from production..user_module_menus_mapping a inner join production..modules_menu_master b on a.module=b.module and a.mnuname=b.mnuname left outer join jctdev..JCT_Menu_Form_Mapping c on b.module = c.module and b.mnuname=c.mnuname where a.module ='ops' and uname='" & Session("Empcode") & "' union select b.description,page_name=REPLACE(REPLACE(REPLACE(ISNULL(REPLACE(REPLACE(REPLACE(ISNULL(c.page_name,''), '~', ''),'/', ''), '\', ''), ''),'-','/'),'*','~'),'$','/'),parent_menu,seq from production..role_module_menus_mapping a inner join production..modules_menu_master b on a.module=b.module and a.mnuname=b.mnuname inner join production..role_user_mapping e on a.role=e.role left outer join jctdev..JCT_Menu_Form_Mapping c on b.module = c.module and b.mnuname=c.mnuname  where a.module ='ops' and uname='" & Trim(Session("Empcode")) & "' order by parent_menu,seq" '"select b.description,page_name=isnull(page_name=replace(replace(replace(isnull(c.page_name,''),'~',''),'/',''),'\',''),''),parent_menu,seq from production..user_module_menus_mapping a inner join production..modules_menu_master b on a.module=b.module and a.mnuname=b.mnuname left outer join jctdev..JCT_Menu_Form_Mapping c on b.mnuname=c.mnuname where a.module ='Costing System' and uname='" & Session("Empcode") & "' union select b.description,page_name=isnull(page_name=replace(replace(replace(isnull(c.page_name,''),'~',''),'/',''),'\',''),''),parent_menu,seq from production..role_module_menus_mapping a inner join production..modules_menu_master b on a.module=b.module and a.mnuname=b.mnuname inner join production..role_user_mapping e on a.role=e.role left outer join jctdev..JCT_Menu_Form_Mapping c on b.mnuname=c.mnuname  where a.module ='Costing System' and uname='" & Trim(Session("Empcode")) & "' order by parent_menu,seq"
