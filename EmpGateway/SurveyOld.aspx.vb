...
Partial Class Survey
    ...
    Protected Sub ApplyBtn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ApplyBtn.Click
        If RdoList.Items.Count > 0 Then
            If RdoList.SelectedValue = "" Then
                Response.Write("<script>alert('Please make your choice! Before Procedding Next')</script>")
                Exit Sub
            End If
        End If
        ...
        If SurveyNo > "0" Then
            ...
            If Label3.Text = "Arrange These" Then
                For sq = 0 To ChkList.Items.Count - 1
                    ...
                    qry = "Insert into jct_emp_survey_Trans(CompanyCode,UserCode,RatedOn,Rating,Coments,SurveyNo,status,Flag,Trans_No,quest_no, ParameterSeqNo) values('JCT00LTD','" & Session("Empcode") & "',getdate(),'" & sq + 1 & "','" & Replace(txtComents.Text, "'", "''") & "'," & SurveyNo & ",'',''," & TransNo & "," & lblQno.Text & "," & ChkList.Items(sq).Value & " )"
                    cmd = New SqlCommand(qry, obj.cn)
                    cmd.ExecuteNonQuery()
            ...
            Else
                Response.Write("<script>alert('Thanks For Rating!! Moving Next To other Questions of the survey')</script>")
                txtComents.Text = ""
                GetSurvey()
                GetCurQuestNo()
                GetParameters()
    ...
    Public Sub GetSurvey()
        ...
        qry = "select distinct subject,confidential_flag, a.survey_no from jct_emp_survey_master a, jct_emp_survey_quest_master b where a.flag='S' and a.auth_flag='A' and a.survey_no=b.survey_no and convert(varchar(5),b.survey_no)+ convert(varchar(5),b.quest_no)    not in (select convert(varchar(5),surveyno)+ convert(varchar(5),quest_no) from jct_emp_survey_trans where status ='' and usercode='" & Session("empcode") & "')"
        cmd = New SqlCommand(qry, obj.cn)
        If Not IsPostBack Then
            ...
            If dr.HasRows = True Then
                While dr.Read()
                    'Added By Neha on 29th March 2010 to pick Survey No from List
    ...
    Public Sub GetParameters()
        qry = "select a.parametername, c.quest, c.imagename, a.survey_no, a.sequence_no from jct_emp_survey_quest_parameter a,jct_emp_survey_master b,jct_emp_survey_quest_master c where a.survey_no=b.survey_no and a.survey_no=c.survey_no and a.quest_no=c.quest_no and a.quest_no=" & lblQno.Text & " and b.subject='" & Me.SurveyList.SelectedItem.Text & "' order by a.sequence_no"
                ...
                While dr.Read()
                    ...
                    Else
                        ...
                        If Label3.Text = "Arrange These" Then
                            ...
                            DrpMoveAt.Items.Add(ChkList.Items.Count)
                        ElseIf Label3.Text = "Check Choice(s)" Then
                            ChkList.Items.Add(lst)
                        ElseIf Label3.Text = "Select One" Then
