...
Partial Class Default4
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            obj.opencn()
            qry = "select task_type from JCT_Emp_Task_Type_Master where company_code='" & Session("Companycode") & "' and status='' order by task_type"
            ...
            obj.opencn()
            qry = "select area from JCT_Emp_Area_Master where company_code='" & Session("Companycode") & "' and status='' order by area"
            cmd = New SqlCommand(qry, obj.cn)
            dr = cmd.ExecuteReader
            ...
            If Request.QueryString.Get("reply") = 1 Or Request.QueryString.Get("reply") = 2 Then
                ...
                obj.opencn()
                qry = "select * from jct_task_log where company_code='" & Session("Companycode") & "' and task_no='" & Request.QueryString.Get("task") & "'"
                cmd = New SqlCommand(qry, obj.cn)
                dr = cmd.ExecuteReader
                If dr.HasRows = True Then
                    While dr.Read()
                ...
                Me.PnlGrid.Collapsed = True
                obj.opencn()
                qry = "select b.empcode + ':-' + b.empname from jct_task_recepients a, jct_empmast_base b where a.company_code=b.company_code and a.company_code='" & Session("Companycode") & "' and a.recp_code=b.empcode and a.flag='t' and task_no='" & Trim(Me.txtTaskNo.Text) & "'"
                cmd = New SqlCommand(qry, obj.cn)
                dr = cmd.ExecuteReader
                ...
                obj.opencn()
                qry = "select b.empcode + ':-' + b.empname from jct_task_recepients a, jct_empmast_base b where a.company_code=b.company_code and a.company_code='" & Session("Companycode") & "' and a.recp_code=b.empcode and a.flag='c' and task_no='" & Trim(Me.txtTaskNo.Text) & "'"
                cmd = New SqlCommand(qry, obj.cn)
                dr = cmd.ExecuteReader
                ...
                Dim cnt As Integer
                obj.opencn()
                qry = "select * from jct_emp_task_file_dir where company_code='" & Session("Companycode") & "' and task_no='" & Trim(Me.txtTaskNo.Text) & "'"
                cmd = New SqlCommand(qry, obj.cn)
                dr = cmd.ExecuteReader
                If dr.HasRows = True Then
                    While dr.Read()
    ...
    Protected Sub DrpArea_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DrpArea.SelectedIndexChanged
        obj.opencn()
        qry = "select subarea from JCT_Emp_Sub_Area_Master where company_code='" & session("Companycode") & "' and area ='" & Trim(Me.DrpArea.Text) & "' and status='' order by subarea"
        cmd = New SqlCommand(qry, obj.cn)
        dr = cmd.ExecuteReader
    ...
    Protected Sub DrpSubArea_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DrpSubArea.SelectedIndexChanged
        obj.opencn()
        ' qry = "select e_mailID from JCT_Emp_Sub_Area_Responsibility a, savior..mistel b where a.res_emp= b.empcode and status = '' and resp_flag='R' and area='" & Trim(Me.DrpArea.Text) & "' and subarea='" & Trim(Me.DrpSubArea.Text) & "' "
        qry = "select a.empcode + ':-' + empname,'a' as seq from jct_empmast_base a, jct_emp_area_master b, deptmast c where a.company_code=b.company_code and b.company_code=c.company_code and b.company_code = '" & session("Companycode") & "' and a.deptcode=b.area_code and a.deptcode=c.deptcode and a.active='y' and a.empcode not in (select res_emp from JCT_Emp_Sub_Area_Responsibility where company_code='" & session("Companycode") & "' and status='' and area = '" & Trim(Me.DrpArea.Text) & "' and subarea='" & Trim(Me.DrpSubArea.Text) & "') and area='" & Trim(Me.DrpArea.Text) & "' union select empcode+ ':-' + empname , 'b' from jct_empmast_base where company_code = '" & session("Companycode") & "' and deptcode not in (select b.area_code from jct_emp_area_master b,  deptmast c where b.company_code=c.company_code and b.company_code = '" & session("Companycode") & "' and b.area_code=c.deptcode and area='" & Trim(Me.DrpArea.Text) & "') order by seq,a.empcode + ':-' + empname"
        cmd = New SqlCommand(qry, obj.cn)
        dr = cmd.ExecuteReader
        Me.ChkFrom.Items.Clear()
        If dr.HasRows = True Then
        ...
        Me.ChkTo.Items.Clear()
        obj.opencn()
        qry = "select a.empcode + ':-' + empname from jct_empmast_base a, JCT_Emp_Sub_Area_Responsibility b where a.company_code=b.company_code and a.company_code = '" & session("Companycode") & "' and a.empcode=res_emp and b.status='' and a.active='y' and area = '" & Trim(Me.DrpArea.Text) & "' and subarea='" & Trim(Me.DrpSubArea.Text) & "' and resp_flag='r' order by a.empcode + ':-' + empname"
        cmd = New SqlCommand(qry, obj.cn)
        dr = cmd.ExecuteReader
        If dr.HasRows = True Then
            While dr.Read()
        ...
        Me.ChkCC.Items.Clear()
        obj.opencn()
        '      qry = "select a.empcode + ':-' + empname,'a' as seq from jct_empmast_base a, jct_emp_area_master b, deptmast c where a.deptcode=b.area_code and a.deptcode=c.deptcode and a.active='y' and a.empcode  in (select res_emp from JCT_Emp_Sub_Area_Responsibility where status='' and area = '" & Trim(Me.DrpArea.Text) & "' and subarea='" & Trim(Me.DrpSubArea.Text) & "' and resp_flag='o') order by seq,a.empcode + ':-' + empname"
        ...
        obj.opencn()
        qry = "select trans_type from JCT_Emp_Trans_Type_Mapping where company_code='" & session("Companycode") & "'  and status='' and area='" & Trim(Me.DrpArea.Text) & "' and subarea='" & Trim(Me.DrpSubArea.Text) & "' "
        cmd = New SqlCommand(qry, obj.cn)
        dr = cmd.ExecuteReader
    ...
    Public Function CheckFileName(ByVal c As String, ByVal Flag As Char)
        ...
        obj.opencn()
        qry = "select * from JCT_Emp_Task_File_Dir a, jct_task_log b where a.company_code=b.company_code and a.company_code = '" & session("Companycode") & "' and a.task_no=b.task_no and a.emp_code=b.emp_code and a.flag='" & Flag & "' and a.file_nm='" & c & "' and a.task_no='" & Trim(Me.txtTaskNo.Text) & "'"
        cmd = New SqlCommand(qry, obj.cn)
        dr = cmd.ExecuteReader
        If dr.HasRows = True Then
            dr.Read()
    ...
    '    Try
    '        obj.opencn()
    '        qry = "insert into JCT_Emp_Task_File_Dir values ('JCT00LTD','','" & Trim(Me.txtTaskNo.Text) & "','" & c & "','A',host_name(),getdate())"
    '        cmd = New SqlCommand(qry, obj.cn)
    '        cmd.ExecuteNonQuery()
    '        obj.closecn()
    ...
    '    Try
    '        obj.opencn()
    '        qry = "insert into JCT_Emp_Task_File_Dir values ('JCT00LTD','','" & Trim(Me.txtTaskNo.Text) & "','" & c & "','R',host_name(),getdate())"
    '        cmd = New SqlCommand(qry, obj.cn)
    '        cmd.ExecuteNonQuery()
    '        obj.closecn()
    '        'Response.Write("<script>alert('Reply Sent Successfully!!')</script>")
    ...
    Protected Sub cmdAttachAssn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdAttachAssn.Click
        If Me.cmdAttachAssn.Text = "Attach File" Then
            ...
            obj.opencn()
            qry = "select max(convert(integer,right(task_no,len(task_no)-5)))+1 from JCT_Emp_Task_File_Dir "
            cmd = New SqlCommand(qry, obj.cn)
            dr = cmd.ExecuteReader
            If dr.HasRows = True Then
                dr.Read()
            ...
            qry = "insert into JCT_Emp_Task_File_Dir values ('" & session("Companycode") & "','" & Session("Empcode") & "','" & Trim(Session("Task")) & "','" & Me.LnkAssign.Text & "','A',host_name(),getdate())"
            cmd = New SqlCommand(qry, obj.cn)
            cmd.ExecuteNonQuery()
            obj.closecn()
        ...
        ElseIf Me.cmdAttachAssn.Text = "Remove File" Then
            ...
            obj.opencn()
            qry = "delete from JCT_Emp_Task_File_Dir where company_code='" & session("Companycode") & "' and emp_code='" & Session("Empcode") & "' and file_nm='" & Me.LnkAssign.Text & "' and task_no='" & Session("Task") & "'"
            cmd = New SqlCommand(qry, obj.cn)
            cmd.ExecuteNonQuery()
            obj.closecn()
        ...
        Try
            ...
            If Me.DrpSelection.Text = "Assign Task" Then
                If Trim(Me.txtJobDescr.Text) = "" Or Trim(Me.txtSub.Text) = "" Then
                    ...
                    obj.opencn()
                    qry = "delete from JCT_Emp_Task_File_Dir where company_code='" & session("Companycode") & "' and emp_code='" & Session("Empcode") & "' and file_nm='" & Me.LnkReply.Text & "' and task_no='" & Session("Task") & "'"
                    cmd = New SqlCommand(qry, obj.cn)
                    cmd.ExecuteNonQuery()
                    obj.closecn()
                ...
                qry = "select max(convert(integer,right(task_no,len(task_no)-5)))+1 from JCT_Task_Log "
                cmd = New SqlCommand(qry, obj.cn)
                ...
                If Trim(Me.LnkAssign.Text) <> "LnkAssign" And Trim(Me.LnkAssign.Text) <> "" Then
                    ...
                    'obj.opencn()
                    qry = "update JCT_Emp_Task_File_Dir set task_no='" & Trim(Me.txtTaskNo.Text) & "' where company_code='" & session("Companycode") & "' and emp_code='" & Session("Empcode") & "' and file_nm='" & Me.LnkAssign.Text & "' and task_no='" & Session("Task") & "'"
                    cmd = New SqlCommand(qry, obj.cn)
                    cmd.Transaction = Trans
                    cmd.ExecuteNonQuery()
                    '  obj.closecn()
                ...
                For i = 0 To Me.ChkTo.Items.Count - 1
                    ...
                    'obj.closecn()
                    ' obj.opencn()
                    qry = "insert into  JCT_Task_Recepients (Company_code,Emp_Code,Task_No,Recp_Code,Flag,history) values ('" & session("Companycode") & "','" & Session("Empcode") & "','" & task_no & "',ltrim(' " & Left(Me.ChkTo.Items(i).Text, InStr(Me.ChkTo.Items(i).Text, ":-") - 1) & "'),'T','')"
                    cmd = New SqlCommand(qry, obj.cn)
                    cmd.Transaction = Trans
                    cmd.ExecuteNonQuery()
                    'obj.closecn()
                ...
                For i = 0 To Me.ChkCC.Items.Count - 1
                    ...
                    'obj.opencn()
                    qry = "insert into  JCT_Task_Recepients (Company_code,Emp_Code,Task_No,Recp_Code,Flag,history) values ('" & session("Companycode") & "','" & Session("Empcode") & "','" & task_no & "','" & Left(Me.ChkCC.Items(i).Text, InStr(Me.ChkCC.Items(i).Text, ":-") - 1) & "','C','')"
                    cmd = New SqlCommand(qry, obj.cn)
                    cmd.Transaction = Trans
                    cmd.ExecuteNonQuery()
                    ' obj.closecn()
            ...
            ElseIf Me.DrpSelection.Text = "Reply" Then
                ...
                'obj.opencn()
                qry = "select e_mailid,name from mistel a, jct_task_log b where a.company_code=b.company_code and a.company_code='" & session("Companycode") & "' and a.empcode=b.emp_code and b.task_no='" & Trim(Me.txtTaskNo.Text) & "' and b.status=''"
                cmd = New SqlCommand(qry, obj.cn)
                cmd.Transaction = Trans
                dr = cmd.ExecuteReader
                If dr.HasRows = True Then
                ...
                ' obj.opencn()
                qry = "update JCT_Task_Log set task_status='" & Trim(Me.DrpTaskStatus.Text) & "', history='" & Replace(Trim(Me.txtHistory.Text), "'", "''") & "' where company_code= '" & session("Companycode") & "'and  task_no='" & Trim(Me.txtTaskNo.Text) & "'"
                cmd = New SqlCommand(qry, obj.cn)
                cmd.Transaction = Trans
                cmd.ExecuteNonQuery()
                'obj.closecn()

                qry = "insert into  JCT_Task_Log_Detail (Company_code,Emp_Code,Task_No, Created_Dt,Task_Status,Reply) values ('" & session("Companycode") & "','" & Session("Empcode") & "','" & Trim(Me.txtTaskNo.Text) & "', getdate(),'" & Trim(Me.DrpTaskStatus.Text) & "','" & replace(Trim(Me.txtReply.Text), "'", "''") & "')"
                cmd = New SqlCommand(qry, obj.cn)
                cmd.Transaction = Trans
                cmd.ExecuteNonQuery()
                ...
                If Trim(Me.DrpTaskStatus.Text) = "Re Assigned" And Request.QueryString.Get("reply") = 1 Then
                    ...
                    qry = "insert into JCT_Task_Log (Company_code,Emp_Code,Task_No, Task_Type,Task_Priority, Area, SubArea, Log_SysDate,Task_Ref, Task_Ref_Date,Subject,Trans_Type,Tran_no,Job_Descr,Task_Status,Due_Date,Authorize,History, Status)  select Company_code,'" & Session("Empcode") & "','" & task_no & "', Task_Type,Task_Priority, Area, SubArea, getdate(),Task_No, Log_SysDate,Subject,Trans_Type,Tran_no,Job_Descr,'Deferred',Due_Date,Authorize,History, 'R' from JCT_Task_Log where task_no='" & Me.txtTaskNo.Text & "' and status=''"
                    cmd = New SqlCommand(qry, obj.cn)
                    cmd.Transaction = Trans
                    cmd.ExecuteNonQuery()
                    ...
                    If Trim(Me.LnkReply.Text) <> "LnkReply" And Trim(Me.LnkReply.Text) <> "" Then
                        ...
                        qry = "update JCT_Emp_Task_File_Dir set task_no='" & task_no & "' where company_code='" & session("Companycode") & "' and emp_code='" & Session("Empcode") & "' and file_nm='" & Me.LnkAssign.Text & "' and (task_no='" & Session("Task") & "' or task_no='" & Trim(Me.txtTaskNo.Text) & "')"
                        cmd = New SqlCommand(qry, obj.cn)
                        cmd.Transaction = Trans
                        cmd.ExecuteNonQuery()
                        '  obj.closecn()
                    ...
                    If frm.Exists = True Then
                        ...
                        '********************
                        obj.opencn()
                        qry = "insert into JCT_Emp_Task_File_Dir values ('" & session("Companycode") & "','" & Session("Empcode") & "','" & Trim(task_no) & "','" & Me.LnkReply.Text & "','R',host_name(),getdate())"
                        cmd = New SqlCommand(qry, obj.cn)
                        cmd.ExecuteNonQuery()
                        obj.closecn()
                ...
                ElseIf Request.QueryString.Get("reply") = 2 Then
                    If Me.ChkAuth.Checked = True Then
                        ...
                        If Trim(Me.DrpTaskStatus.Text) = "Cancelled" Then
                            qry = "update JCT_Task_Log set task_status='" & Trim(Me.DrpTaskStatus.Text) & "', authorize='Y', history='" & Me.txtHistory.Text & "', status='C' where company_code='" & session("Companycode") & "' and task_no='" & Trim(Me.txtTaskNo.Text) & "'"
                        Else
                            qry = "update JCT_Task_Log set task_status='" & Trim(Me.DrpTaskStatus.Text) & "', authorize='Y', history='" & Me.txtHistory.Text & "' where company_code='" & session("Companycode") & "' and task_no='" & Trim(Me.txtTaskNo.Text) & "'"
                        End If
                        ...
                        'Else
                        '    SubVal = "Re:" & Me.txtSub.Text
                        '    qry = "update JCT_Task_Log set task_status='" & Trim(Me.DrpTaskStatus.Text) & "', history='" & Me.txtHistory.Text & "' where task_no='" & Trim(Me.txtTaskNo.Text) & "'"
                    End If
                    ...
                    'obj.opencn()
                    qry = "select e_mailid,name from mistel a, jct_task_recepients b where a.company_code=b.company_code and a.company_code='" & session("Companycode") & "' and a.empcode=b.recp_code and b.task_no='" & Trim(Me.txtTaskNo.Text) & "' and b.flag='T'"
                    cmd = New SqlCommand(qry, obj.cn)
                    cmd.Transaction = Trans
                    dr = cmd.ExecuteReader
                    If dr.HasRows = True Then
            ...
            ElseIf Me.DrpSelection.Text = "Close" Then
                ...
                If Me.ChkAuth.Checked = True Then
                    ...
                    qry = "update JCT_Task_Log set task_status='" & Trim(Me.DrpTaskStatus.Text) & "', authorize='Y', history='" & Me.txtHistory.Text & "' where task_no='" & Trim(Me.txtTaskNo.Text) & "'"
                Else
                    SubVal = "Re:" & Me.txtSub.Text
                    qry = "update JCT_Task_Log set task_status='" & Trim(Me.DrpTaskStatus.Text) & "', history='" & Me.txtHistory.Text & "' where task_no='" & Trim(Me.txtTaskNo.Text) & "'"
                End If
                ...
                qry = "insert into  JCT_Task_Log_Detail (Company_code,Emp_Code,Task_No, Created_Dt,Task_Status,Reply) values ('" & session("Companycode") & "','" & Session("Empcode") & "','" & Trim(Me.txtTaskNo.Text) & "', getdate(),'" & Trim(Me.DrpTaskStatus.Text) & "','" & Trim(Me.txtReply.Text) & "')"
                cmd = New SqlCommand(qry, obj.cn)
                cmd.Transaction = Trans
                cmd.ExecuteNonQuery()
    ...
    Protected Sub cmdAttachReply_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdAttachReply.Click
        Try
            If Me.cmdAttachReply.Text = "Attach File" Then
                ...
                If nwFld.Exists = False Then nwFld.Create()
                obj.opencn()
                qry = "insert into JCT_Emp_Task_File_Dir values ('" & session("Companycode") & "','" & Session("Empcode") & "','" & Trim(Session("Task")) & "','" & Me.LnkReply.Text & "','R',host_name(),getdate())"
                cmd = New SqlCommand(qry, obj.cn)
                cmd.ExecuteNonQuery()
                obj.closecn()
    ...
    Protected Sub cmdReAssign_Click(ByVal sender As Object, ByVal e As System.EventArgs) ''Handles cmdReAssign.Click
        ...
        Try
            ...
            ' obj.opencn()
            qry = "update JCT_Task_Log set task_status='Re Assigned', history='" & Me.txtHistory.Text & "' where task_no='" & Trim(Me.txtTaskNo.Text) & "'"
            cmd = New SqlCommand(qry, obj.cn)
            cmd.Transaction = Trans
            cmd.ExecuteNonQuery()
            'obj.closecn()
        ...
        With MailSmpt
            ...
            If dr.HasRows = True Then
                dr.Read()
                If dr.Item(0) Is System.DBNull.Value Then
                    .From = "neha.srivastava@jctltd.com"
                Else
                    .From = Trim(dr.Item(0))
                End If
            Else
