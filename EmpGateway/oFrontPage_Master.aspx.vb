...
Partial Class FrontPage_Master
    ...
    Protected Sub cmdAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdAdd.Click
        ...
        If cmdAdd.Text = "Save" Then
            ...
            Else
                If Me.ddlItemcateg.SelectedItem.Text = "Medical Tips" Or Me.ddlItemcateg.SelectedItem.Text = "General Tips" Then
                    SQLPASS = "insert into jct_fap_shared_area(CompanyCode,UserCode,ItemCode,SDescription,LDescription,itemcat,ParentItem,URL,createddt,efffrom,effto,STATUS,doc_type) values('" & Session("Companycode") & "','" & Session("Empcode") & "','" & Me.txtItemCode.Text & "','" & Me.txtSDesc.Text & "','" & Me.txtLDesc.Text & "','" & Me.ddlItemcateg.SelectedValue & "','" & Me.txtPItemCode.Text & "','" & Me.txtDocumentIconImage.Text & "',getdate(),'" & Me.txtEffFrom.Text & "','" & Me.txtEffTo.Text & "','','" & Me.TxtDocType.Text & "')"
                Else
    ...
    Protected Sub LoadGrid_parent()
        If Me.ddlItemcateg.SelectedItem.Text = "Medical Tips" Or Me.ddlItemcateg.SelectedItem.Text = "General Tips" Then
            SQLPASS = "select distinct parentitem,itemcode from jct_fap_shared_area where status='' and itemcat='" & Me.ddlItemcateg.SelectedValue & "' AND DateDiff(DD,GETDATE(),EFFTO)>=0"
        Else
            SQLPASS = "SELECT  distinct PARENTITEMCODE,[Code]=ITEMCODE, [Short Desc]=SDESCRIPTION from jctdev..jct_fap_item_master1 WHERE status='A' AND DateDiff(DD,GETDATE(),EFFTO)>=0   "
        End If
    ...
    Protected Sub LoadGrid_DocType()
        ...
        SQLPASS = "select 'GeneralArea' as [Document Type]  union select 'MedicalTips' as [Document Type]  union SELECT distinct Document_Type as [Document Type] from jctdev..jct_fap_item_master1 WHERE status='A' AND DateDiff(DD,GETDATE(),EFFTO)>=0   "
