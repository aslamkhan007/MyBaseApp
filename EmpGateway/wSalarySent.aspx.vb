...
Partial Class SalarySent
    ...
    Protected Sub cmdapply_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdapply.Click
        ...
        If Me.ChkClerks.Checked = True Then
            obj.opencn()
            qry = "insert into jct_emp_Salary_Update select 'JCT00LTD','" & Session("Empcode") & "','" & Trim(Me.LblYear.Text) & Right("0" & Me.DrpMonth.SelectedIndex + 1, 2) & "',a.catg,'" & Trim(Me.DtClerk.SelectedDate) & "', a.empcode,null,null,'" & ddlSal.SelectedValue & "' from jct_empmast_base a, JCT_Emp_Catg_Desg_Mapping b where a.catg=b.catg and a.active='y' and b.status='' and designation='Clerks'"
            cmd = New SqlCommand(qry, obj.cn)
            cmd.ExecuteNonQuery()
            obj.closecn()
    ...
    Protected Sub DrpMonth_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DrpMonth.SelectedIndexChanged
        obj.opencn()
        qry = "select distinct designation from jct_emp_Salary_Update a, JCT_Emp_Catg_Desg_Mapping b where a.catg=b.catg and b.status='' and monthyear='" & Trim(Me.LblYear.Text) & Right("0" & Me.DrpMonth.SelectedValue, 2) & "' and type='" & ddlSal.SelectedValue & "'"
        cmd = New SqlCommand(qry, obj.cn)
        dr = cmd.ExecuteReader
        If dr.HasRows = True Then
            While dr.Read()
    ...
    Protected Sub SendSalarySMS(ByVal desg As String)
        ...
        If ddlSal.SelectedItem.Text = "Salary" Then
            msg = "Your Salary for the Month of '" & Me.DrpMonth.SelectedItem.Text & "', " & Me.LblYear.Text & " has been transferred to your bank account."
        ElseIf ddlSal.SelectedItem.Text = "Allowance" Then
            msg = "Your Allowance for the Month of '" & Me.DrpMonth.SelectedItem.Text & "', " & Me.LblYear.Text & " has been transferred to your bank account."
        End If
        ...
        Try
            ...
            End If
            sm.SendSMS(Session("CompanyCode"), Session("EmpCode"), receiver, msg, ddlSal.Text & " Sent")
    ...
    Protected Sub ddlSal_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlSal.SelectedIndexChanged
        DrpMonth_SelectedIndexChanged(sender, e)
    End Sub
End Class
