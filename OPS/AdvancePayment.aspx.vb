...
Partial Class AdvancePayment
    ...
    Protected Sub BtnRefresh_Click(sender As Object, e As System.EventArgs) Handles BtnRefresh.Click
        ...
        SqlPass = "EXEC ARDB..PartyAdvanceBalance '" & Trim(Date.Now.ToShortDateString) & "',  '" & Trim(Date.Now.ToShortDateString) & "' "
    ...
    Protected Sub BtnFetch_Click(sender As Object, e As System.EventArgs) Handles BtnFetch.Click
        SqlPass = "SELECT  cust_no AS CustNo ,  UPPERcust_name) AS CustomerName,  UPPER(city) AS City ,  CONVERT(NUMERIC(30),amt *-1) AS Amount FROM    ARDB..JCT_OPS_TRAIL_BALANCE      WHERE ISNULL(amt,0)<0                ORDER BY cust_no "
        PassString(SqlPass)
    End Sub
