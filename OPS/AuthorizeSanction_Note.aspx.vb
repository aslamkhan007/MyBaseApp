...
'25 MARCH 2016 
                                    qry = "SELECT  STUFF(( SELECT  ',' + s.E_MailID FROM    ( SELECT    1 ID , E_MailID FROM      jct_ops_material_request a ,MISTEL c WHERE     RequestID = " + ViewState("ID") + " AND a.userid = c.empcode UNION SELECT    1 id ,E_MailID FROM      dbo.jct_ops_material_request a , MISTEL c WHERE RequestID = " + ViewState("ID") + " AND a.sales_person = REPLACE(c.empcode, '-', '')) s FOR XML PATH('') ), 1, 1, '') AS CSV "
...
FinalNotify = objFun.FetchValue(qry, con, Tran)
                            ...
                            If AreaCode = 9999 Then
                                ' objFun.SendMailOPS(Body, "", SalePersonEmail, Session("Empcode"), "ashish@jctltd.com", "Ashish@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com,hitesh@jctltd.com", "Your orderNO :-""  SortNo :-  ""' Shade :-  " & Shade & " was Reequested be Re-Planned in " & Area & " section and the request was generated by  " & UserName)
                            Else ' Else part executed for all sanction note other than Exceptions
                                Try
                                    ...
                                    If (AreaCode = 1014) Then
                                        SendMail("", SendMailTo, AuthorizedBy, "Ashish@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com", "SanctionNote :-" & SanctionNote & "  has been " & ddlAction.SelectedItem.Text & "", SanctionNote, CurrentUserLevel, MaxAuthLevel, FinalNotify, ddlAction.SelectedItem.Text)
                                    Else
                                        SendMail(Body, Body1, Body3, "", SendMailTo, AuthorizedBy, "hitesh@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com", "SanctionNote :-" & SanctionNote & "  has been " & ddlAction.SelectedItem.Text & "", SanctionNote, CurrentUserLevel, MaxAuthLevel, FinalNotify, ddlAction.SelectedItem.Text)
                                    End If
    ...
    Private Sub SendMail(Body__1 As String, Body2 As String, body3 As String, RaisedBy_Email As String, [to] As String, cc As String, bcc As String, Subject As String, SanctionNote As String, CurrentLevel As Int16, MaxLevel As Int16, NotifyAllList As String, Action As String)
        Try
            ...
            bcc = "hitesh@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com,sandeepr@jctltd.com,hiren@jctltd.com"
            ...
            If [to] = "rohits@jctltd.com" Or [to] = "Ashish@jctltd.com" Then
                ...
                from = "approvals@jctltd.com"
                If Area = "1014" Then
                    cc = NotifyAllList & ",hitesh@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com,skj@jctltd.com,hitesh@jctltd.com"
                ElseIf Area <> "1059" Then
                    cc = NotifyAllList & ",hitesh@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com"
                ElseIf Area = "1059" Then
                    cc = NotifyAllList & ",hitesh@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com,skj@jctltd.com"
                End If
...
Dim amountInvolved As String
                        amountInvolved = String.Empty
...
                            If i = 0 Then
                                For J = 0 To GrdSanctionNoteDetail.Rows(0).Cells.Count - 1

                                    If (J <> GrdSanctionNoteDetail.Rows(0).Cells.Count - 1) Then 'not to include Amount Involved column
...
                                        GridHeader += "<th> " & .HeaderRow.Cells(J).Text & "</th>"
                                    Else
                                        amountInvolved = .HeaderRow.Cells(J).Text
                                    End If
                                End If
                                If (J <> GrdSanctionNoteDetail.Rows(0).Cells.Count - 1) Then 'not to include Amount Involved column
...
                                End If
                                amountInvolved = .Rows(i).Cells(J).Text
                            Next
...
If Area = "1048" Then
                    sb.AppendLine("</br></br><b> Amount Involved :- " & amountInvolved & "</b><br/><br/>")
                End If



                sb.AppendLine("<br/><br/>Authorization Remarks are Shown below: <br/><br/>")
                sb.AppendLine("<head>")
                sb.AppendLine("<style type=""text/css"">")
                sb.AppendLine(" table.gridtable {font-family: verdana,arial,sans-serif;font-size:11px;color:#333333;border-width: 1px;border-color: #666666;border-collapse: collapse;}")
                sb.AppendLine(" table.gridtable th {border-width: 1px;padding: 8px;border-style: solid;border-color: #666666;background-color: #dedede;}")
    ...
    Private Sub SendMail(RaisedBy_Email As String, [to] As String, cc As String, bcc As String, Subject As String, SanctionNote As String, CurrentLevel As Int16, MaxLevel As Int16, NotifyAllList As String, Action As String)
        ...
        Dim FreightPaidBy As String = String.Empty
        Dim FreightType As String = String.Empty
        Dim ActionToBeTaken As String = String.Empty

        Dim RequestType As String = String.Empty
        Dim ExpectedArrivalDate As String = String.Empty
...
'Added on 31 MArch 2016
        sql = "SELECT  STUFF(( SELECT  ',' + s.E_MailID FROM    ( SELECT    b.E_MailID FROM      dbo.jct_ops_material_request a INNER JOIN dbo.MISTEL b ON a.userid = b.empcode WHERE     RequestID = " + SanctionNote + " UNION SELECT    b.E_MailID FROM  dbo.jct_ops_material_request a INNER JOIN dbo.MISTEL b ON a.sales_person = REPLACE(b.empcode, '-', '') WHERE RequestID = " + SanctionNote + " ) s FOR XML PATH('') ), 1, 1, '') AS CSV "
...
Dim Request_by As String = objFun.FetchValue(sql).ToString()
        ...
        If (Left(Action, 1) = "A") Then
            ...
            'Dr.Close()
            Reason = String.Empty
            'Ashish Sharma sql = "SELECT isnull(a.invoice_no,'') as invoice_no,isnull(a.item_no,'') as item_no,isnull(a.customer,'') as customer,isnull(b.empname,'') as sales_person,isnull(Convert(numeric(12,2),a.invoice_qty),0) as invoice_qty,isnull(Convert(Numeric(12,2),a.ret_qty),0) as ret_qty ,isnull(Substring(a.FlagAuth,2,999),'') as FlagAuth,AuthStatus ,reason,isnull(description,'') as description ,FreightValue,a.Freight_by as FreightPaidBy,CONVERT(VARCHAR,a.invoice_date,103) AS invoice_date FROM dbo.jct_ops_material_request a INNER JOIN dbo.JCT_EmpMast_Base b ON a.sales_person=REPLACE(b.empcode,'-','') WHERE a.AuthStatus ='P' and a.RequestID=" + SanctionNote
            'comented on 17 Mov 2015 sql = "SELECT isnull(a.invoice_no,'') as invoice_no,isnull(a.item_no,'') as item_no,isnull(a.customer,'') as customer,isnull(b.empname,'') as sales_person,isnull(Convert(numeric(12,2),a.invoice_qty),0) as invoice_qty,isnull(Convert(Numeric(12,2),a.ret_qty),0) as ret_qty ,isnull(Substring(a.FlagAuth,2,999),'') as FlagAuth,AuthStatus ,reason,isnull(description,'') as description ,FreightValue,a.Freight_by as FreightPaidBy,CONVERT(VARCHAR,a.invoice_date,103) AS invoice_date,SalePrice,ValueInvolved,RequestType,ActionToBeTaken,FreightType,ExpectedArrivalDt FROM dbo.jct_ops_material_request a INNER JOIN dbo.JCT_EmpMast_Base b ON a.sales_person=REPLACE(b.empcode,'-','') WHERE a.AuthStatus ='P' and a.RequestID=" + SanctionNote
            sql = "SELECT isnull(a.invoice_no,'') as invoice_no,isnull(a.item_no,'') as item_no,isnull(a.customer,'') as customer,isnull(b.empname,'') as sales_person,isnull(Convert(numeric(12,2),a.invoice_qty),0) as invoice_qty,isnull(Convert(Numeric(12,2),a.ret_qty),0) as ret_qty ,isnull(Substring(a.FlagAuth,2,999),'') as FlagAuth,AuthStatus ,reason,isnull(description,'') as description ,FreightValue,a.Freight_by as FreightPaidBy,CONVERT(VARCHAR,a.invoice_date,103) AS invoice_date,SalePrice,RequestType,ActionToBeTaken,FreightType,ExpectedArrivalDt FROM dbo.jct_ops_material_request a INNER JOIN dbo.JCT_EmpMast_Base b ON a.sales_person=REPLACE(b.empcode,'-','') WHERE a.AuthStatus ='P' and a.RequestID=" + SanctionNote
            ...
            If (Dr.HasRows) Then
                ...
                While (Dr.Read())
                    ...
                    ExpectedArrivalDate = Dr("ExpectedArrivalDt").ToString
                    ActionToBeTaken = Dr("ActionToBeTaken").ToString
            ...
            sql = "SELECT FreightPaidby from JCT_OPS_MATERIAL_REQUEST_FINAL_AUTHORIZATION WHERE REQUESTID=" + SanctionNote
            FreightPaidBy = String.Empty
            Dr = objFun.FetchReader(sql)
            If (Dr.HasRows) Then
                ...
                While (Dr.Read())
                    ...
                    FreightPaidBy = Dr("FreightPaidby").ToString
            ...
            sb.AppendLine("<h2>Request Type :-</h2> " + RequestType + "<br /><br/>")

            sb.AppendLine("<b>Expected Arrival Date : </b>" + ExpectedArrivalDate + "<br /><br/>")

            sb.AppendLine("<b>Action To Be Taken : </b>" + ActionToBeTaken + "<br /><br/>")

            sb.AppendLine("<b>Freight Type :</b> " + FreightType + "<br /><br/>")

            sb.AppendLine("<b>Freight Paid By :</b> " + FreightPaidBy + "<br /><br/>")
            ...
            bcc = "hitesh@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com"
        ...
        ElseIf (Left(Action, 1) = "C") Then
            ...
            bcc = "hiren@jctltd.com,sandeepr@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com,hitesh@jctltd.com"
    ...
    Private Sub EMail(ByVal SanctionNote As String)
        ...
        Try
            ...
            sb.AppendLine("Details are Shown below : <br/><br/>")
            sb.AppendLine("<table class=gridtable>")
            sb.AppendLine("<tr><th> Invoice No</th> <th> Sort</th> <th> Customer</th> <th> Sale Person</th> <th> Invoice Qty</th> <th>Invoice Date</th> <th> Return Qty</th>  <th> Auth. Pending At</th><th> Freight Paid By</th>  <th> Sale Price</th> <th> ValueInvolved</th>  </tr>")
            ...
            If (Dr3.HasRows) Then
                While (Dr3.Read())
                    ...
                    'Comented by Ashish sb.AppendLine("<tr> <td> " & Dr3(0).ToString & " </td> <td> " & Dr3(1).ToString & "  </td>  <td> " & Dr3(2).ToString & "</td>  <td>" & Dr3(3).ToString & " </td>  <td>" & Dr3(4).ToString & " </td> <td>" & Dr3("invoice_date").ToString() & "</td>  <td>" & Dr3(5).ToString & "</td>  <td>ROHIT SERU</td><td>" & Dr3(7).ToString & "</td> </tr> ")
                    sb.AppendLine("<tr> <td> " & Dr3(0).ToString & " </td> <td> " & Dr3(1).ToString & "  </td>  <td> " & Dr3(2).ToString & "</td>  <td>" & Dr3(3).ToString & " </td>  <td>" & Dr3(4).ToString & " </td> <td>" & Dr3("invoice_date").ToString() & "</td>  <td>" & Dr3(5).ToString & "</td>  <td>ROHIT SERU</td><td>" & Dr3(7).ToString & "</td> <td>" & Dr3("SalePrice").ToString & "</td> <td>" & Dr3("ValueInvolved").ToString & "</td>  </tr> ")
            ...
            sb.AppendLine("<table class=gridtable>")
            sb.AppendLine("<tr><th> Gr No.</th> <th> Gr Date</th> </tr>")

            sql = "SELECT  isnull(GrNo,'') as GrNo,isnull(GrDate,'') as GrDate FROM JCT_OPS_MATERIAL_REQUEST_FINAL_AUTHORIZATION WHERE REQUESTID=" & Convert.ToInt16(SanctionNote)
            Dim cmd4 As SqlCommand = New SqlCommand(sql, obj3.Connection())

            'Dim FreightVal As Double = 0.0

            Dim Dr4 As SqlDataReader = cmd4.ExecuteReader()
            If (Dr4.HasRows) Then
                While (Dr4.Read())
                    'FreightVal += Dr4("FreightValue")
                    sb.AppendLine("<tr> <td> " & Dr4(0).ToString & " </td> <td> " & Dr4(1).ToString & "  </td>  </tr> ")
            ...
            'Comented by Ashish Sharma sql = "SELECT distinct DESCRIPTION,reason,FreightValue FROM dbo.jct_ops_material_request  WHERE RequestID=" + SanctionNote
            sql = "SELECT distinct a.DESCRIPTION,a.reason,a.FreightValue,a.RequestType,a.ActionToBeTaken,a.FreightType,CASE a.RequestType WHEN 'Material Return' THEN 'N.A' else isnull(FreightPaidby,'Not Mentioned Yet') end as FreightPaidby,ExpectedArrivalDt  FROM dbo.jct_ops_material_request a left outer join JCT_OPS_MATERIAL_REQUEST_FINAL_AUTHORIZATION b on a.RequestID=b.requestID  WHERE a.RequestID=" + SanctionNote
            ...
            If (Dr3.HasRows) Then
                ...
                While (Dr3.Read())
                    ...
                    sb.AppendLine("<b>RequestType </b> : " + Dr3("RequestType").ToString().ToUpper())
                    sb.AppendLine("<br /><br />")
                    ...
                    sb.AppendLine("<b>FreightType </b> : " + Dr3("FreightType").ToString().ToUpper())
                    sb.AppendLine("<br /><br />")

                    sb.AppendLine("<b>ActionToBeTaken </b> : " + Dr3("ActionToBeTaken").ToString().ToUpper())
                    sb.AppendLine("<br /><br />")

                    sb.AppendLine("<b>ExpectedArrivalDt </b> : " + Dr3("ExpectedArrivalDt").ToString().ToUpper())
                    sb.AppendLine("<br /><br />")


                    sb.AppendLine("Freight Paid by  : " + Dr3("FreightPaidby").ToString())
                    sb.AppendLine("<br /><br />")
            ...
            mail.CC.Add("hiren@jctltd.com")
            mail.CC.Add("sandeepr@jctltd.com")
            mail.CC.Add("hitesh@jctltd.com")


            
            mail.CC.Add("skj@jctltd.com")
            mail.CC.Add(" naunihal@jctltd.com")
            mail.CC.Add("ashokjoshi@jctltd.com")
	   mail.CC.Add("rameshs@jctltd.com")



            mail.CC.Add("ranjitsaini@jctltd.com")
            mail.CC.Add("dheerajarora@jctltd.com")
...
'                                SendMail(Body, Body1, Body3, RaisedBy, SendMailTo, "ashish@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com,hitesh@jctltd.com", "Ashish@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com,hitesh@jctltd.com", "This SanctionNote :-" & SanctionNote & "  has been " & ddlAction.SelectedItem.Text & "", SanctionNote, CurrentUserLevel, MaxAuthLevel, FinalNotify, ddlAction.SelectedItem.Text)
'                            End If
'                            RefreshLists()
...
'        '                'CurrentUserLevel = objFun.FetchValue(qry, con, Tran)
'        '                If AreaCode = 9999 Then
'        '                    objFun.SendMailOPS(Body, OrderNo, SalePersonEmail, Session("Empcode"), "ashish@jctltd.com", "Ashish@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com,hitesh@jctltd.com", "Your orderNO :-""  SortNo :-  ""' Shade :-  " & Shade & " was Reequested be Re-Planned in " & Area & " section and the request was generated by  " & UserName)
'        '                Else
'        '                    'objFun.SendMailOPS(Body, OrderNo, SalePersonEmail, Session("Empcode"), "rahuljindal@jctltd.com,rashpal@jctltd.com,karunarora@jctltd.com,khushwinder@jctltd.com,neeraj@jctltd.com,sobti@jctltd.com", "Ashish@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com,hiren@jctltd.com,sandeepr@jctltd.com,hitesh@jctltd.com", "Your orderNO :-""  SortNo :-  " & Item & "' Shade :-  " & Shade & " was Removed from  Dyeing & Finishing Plan")
'        '                End If
'        '                Tran.Commit()
'        '                RefreshLists()
...
'        bcc = "hiren@jctltd.com,sandeepr@jctltd.com,ashish@jctltd.com,hitesh@jctltd.com"
'        ' [to] = ("ashish@jctltd.com")
'        'Email Address of Receiver
'        'cc = "ashish@jctltd.com,jagdeep@jctltd.com,hitesh@jctltd.com"
