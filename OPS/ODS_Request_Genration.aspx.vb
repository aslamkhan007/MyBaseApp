...
Partial Class OPS_ODS_Request_Genration
    ...
    Protected Sub CmdApply_Click(sender As Object, e As System.EventArgs) Handles CmdApply.Click
        ...
        Try
            ...
            If ddlRequestType.SelectedItem.Text = "ODS" Then
                qry = " exec Jct_Ops_ODS_Insert_HDR '" & Session("Empcode") & "','" & Area & "','" & txtSubject.Text & "','" & txtDescription.Text & "','" & Request.ServerVariables("REMOTE_ADDR") & "','" & SanctionID & "','" & ddlPlant.SelectedItem.Text & "','" & ddlRequestType.SelectedItem.Text & "','" & ddlMode.SelectedItem.Text & "','" & ddlFreightType.SelectedItem.Text & "','" & ddlDocsSentTo.SelectedItem.Text & "','" & LblAddress.Text & "','" & txtTransportDetail.Text & "','" & txtRemarks.Text & "'"
            Else
                qry = " exec Jct_Ops_ODS_Insert_HDR '" & Session("Empcode") & "','" & Area & "','" & txtSubject.Text & "','" & txtDescription.Text & "','" & Request.ServerVariables("REMOTE_ADDR") & "','" & SanctionID & "','" & ddlPlant.SelectedItem.Text & "','" & ddlRequestType.SelectedItem.Text & "','" & ddlMode.SelectedItem.Text & "','" & ddlFreightType.SelectedItem.Text & "','" & ddlDocsSentTo.SelectedItem.Text & "','" & LblAddress.Text & "','" & txtTransportDetail.Text & "','" & txtRemarks.Text & "'"
            End If
            ...
            With GrdTempValues
                ...
                If AreaSelected(0).ToUpper <> "EXCESS STOCK" Then
                    ...
                    qry = "Jct_Ops_Ods_Request_SalePersonHierarchy_Insert "
                    'cmd = New SqlCommand(qry, obj.Connection)
                    ...
                    cmd.Parameters.Add(New SqlParameter("@SanctionNote", SqlDbType.VarChar, 15)).Value = SanctionID
            ...
            qry = "Jct_Ops_Expdoc_Insert_request_generate"
            con1.Open()
            ...
            cmd.Parameters.Add("@SanctionNoteID", SqlDbType.VarChar, 14).Value = SanctionID
    ...
    Private Sub GenrateMail(Body As String, Body1 As String, Body2 As String, Body3 As String, OrderNo As String, SalesPerson_Name As String, [to] As String, cc As String, bcc As String, Subject As String, ID As String, PendingAt As String, DespatchMode As String, FreightType As String, DocsLocation As String, GenratedBy As String, GenratedbyEmail As String)
        ...
        sb.AppendLine("<table class=gridtable>")
        sb.AppendLine("<tr><th> NOTIER NAME </th> <th> ADDRESS1 </th> <th> ADDRESS2 </th> <th> ADDRESS3 </th> <th> CITY </th>   <th> STATE </th>  <th> COUNTRY </th> <th> BUYERS PO </th> <th> PAYMENT TERM </th> <th> DESTINATION COUNTRY </th>  <th> DESPATCH COUNTRY </th>  </tr>")
        qry = "Select notifyname, notifyadd1, notifyadd2,notifyadd3,notifycity,notifystate,notifycountry,buyerPo,PaymentTerm,DestinationPort,DespatchPort  from Jct_Ops_Expdoc_request_generate where SanctionNoteID ='" & SanctionID1 & "' "
        con1.Open()
        cmd = New SqlCommand(qry, con1)
        dr = cmd.ExecuteReader
        While (dr.Read())
            sb.AppendLine("<tr> <td> " & dr(0).ToString & " </td> <td> " & dr(1).ToString & "  </td>  <td> " & dr(2).ToString & "</td>  <td>" & dr(3).ToString & " </td>  <td>" & dr(4).ToString & " </td> <td>" & dr(5).ToString & " </td> <td>" & dr(6).ToString & " </td> <td>" & dr(7).ToString & " </td> <td>" & dr(8).ToString & " </td> <td>" & dr(9).ToString & " </td> <td>" & dr(10).ToString & " </td> </tr> ")
        End While
        dr.Close()
        con1.Close()
        sb.AppendLine("</table>")
                ...
                For i = 0 To .Rows.Count - 1
                    If CType(.Rows(i).FindControl("chkBox"), CheckBox).Checked = True Then
                        ...
                        Else
                            ...
                            qry = "exec Jct_Ops_Transfer_Request_Intermediate_Insert '" & Session("Empcode") & "','" & Trim(.Rows(i).Cells(3).Text) & "','" & Trim(.Rows(i).Cells(5).Text) & "','" & Trim(.Rows(i).Cells(3).Text) & "','" & Trim(.Rows(i).Cells(2).Text) & "','" & Trim(.Rows(i).Cells(7).Text) & "','" & Trim(.Rows(i).Cells(5).Text) & "','" & Trim(.Rows(i).Cells(6).Text) & "','" & Trim(.Rows(i).Cells(10).Text) & "','A','" & Trim(.Rows(i).Cells(11).Text) & "'"
                        End If
                        'objfun.InsertRecord(qry)
                        cmd = New SqlCommand(qry, obj.Connection)
                        cmd.ExecuteNonQuery()
