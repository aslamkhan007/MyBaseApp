...
Partial Class PackedStock
    ...
    Protected Sub ItemGroup()
        ...
        If RadioButtonList1.SelectedValue = "ALL" Then
            SqlPass = "SELECT    item_group_no AS ItemGroup ,LOT_NO AS LotNo, DATEDIFF(dd,recd_date,GETDATE()) AS Ageing,Wh_no AS Godown, ISNULL(Shade,'') AS Shade ,stock_no AS SortNo ,variant_no AS Variant , ISNULL(CONVERT(VARCHAR,CONVERT(NUMERIC(8,2),(qty_recd ))),'') AS Qty ,   REPLACE(CONVERT(VARCHAR(11), recd_date, 106), ' ', '-')   AS PackedDate , SalePerson AS SalePerson , CustomerCode AS CustomerCode , UPPER(order_no) AS SaleOrderNo ,ISNULL(CustomerName, '') AS CustomerName , ISNULL(CONVERT(varchar,OrderDate,103),'')  AS OrderDate   FROM       JCT_Pack_Stock_UptoDate_MIS ORDER BY item_group_no,Stock_no"
 ...
 ElseIf RadioButtonList1.SelectedValue = "ITEM GROUP WISE" Then
        ...
        ElseIf RadioButtonList1.SelectedValue = "SALEPERSON WISE AGEING" Then
            ...
            ' "UNION SELECT '[~GRAND TOTAL ~]',    CONVERT(VARCHAR,SUM(CONVERT(NUMERIC(10, 2),ISNULL(qty_recd,0)))),'','','','','','','','','','','','' FROM JCT_Pack_Stock_UptoDate_MIS ORDER BY SalePerson "


            Sqlpass = "SELECT  SUBSTRING(Segment,3, 10 ) as SEGMENT, Type ,SalePerson , [0-30] ,[31-60] ,[61-90] , [91-120] ,[121-150] , [151-180] ,[181-210] , [211-240] ,  [241-270] ,     [271-300] ,     [301-330] ,      [331-365] ,     [>365] , Total FROM    PackedStockAgeing WHERE   SalePerson IS NOT NULL ORDER BY Segment ,SalePerson"
    ...
    Function CreateExcelFile(ByVal dt As DataTable) As Boolean
        ...
        Dim sExcelFile As String = Server.MapPath("DnV_Detail.html")
