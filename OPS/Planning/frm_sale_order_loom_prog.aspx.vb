...
Partial Class frm_sale_order_loom_prog
    ...
    Protected Sub cmdFetch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdFetch.Click
        ...
        sqlpass = "exec jct_ops_mktg_loom_programme '" & Right(ddlyrmth.Text, 6) & "','" & ddllocation.SelectedItem.Text & "' "
        ...
        Else
            FMsg.CssClass = "errormsg"
            FMsg.Message = "Sales Orders not Freezed"
            FMsg.Display()
        End If
        cn.Close()
        sqlpass = ""
    ...
    Public Sub BindData1()
        ...
        Sqlpass = "SELECT team_code 'Team',sale_person 'Sale_Person',order_no 'OrderNo', order_srl_no 'SrlNo ',convert(varchar(10),order_dt,103) 'OrderDt', convert(varchar(10),req_dt,103) 'NeedDt',cust_name 'Customer',blend 'Blend',cloth_type 'Cloth_type',item_no 'ItemNo',variant 'Variant',convert(numeric(12),order_qty) 'OrderQty',convert(numeric(12),freezed_plan_qty) 'PlanQty' FROM jct_ops_mktg_order_forloomprog  where host_id  = host_id() and yearmonth = '" & Right(ddlyrmth.Text, 6) & "' and  location = '" & ddllocation.SelectedItem.Text & "'"
    ...
    Protected Sub cmdexcel1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdexcel1.Click
        'Try
        GridViewExportUtil.Export("Orders_loomprog.xls", Me.grdGrid1)
