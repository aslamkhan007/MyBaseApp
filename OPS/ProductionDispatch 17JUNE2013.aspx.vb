...
Partial Class ProductionDispatch
    ...
    Protected Sub BtnFetch_Click(sender As Object, e As System.EventArgs) Handles BtnFetch.Click
        ...
        If Session("empcode").ToString = "P-03055" Then
            SqlPass = "SELECT Plant ,    OpeningStock ,     TillPacking AS MTDPacking ,  TillDispatch AS MTDDispatch,  MonDispVal AS [Month Dispatch Value],TillUnis AS MTDUnis ,YP AS '" & R & " - Packing'  ,YD AS '" & R & " - Dispatch'  , LDV AS '" & R & " - Dispatch Value'  ,   Balance FROM SHP..JCT_OPS_PRODUCTION_DISPATCH_TABLE_PG  WHERE   Plant IN ( 'TAFFETA', 'YARN', 'GARMENTS', 'SYNTHETIC', 'COTTON','[TOTAL]' ) ORDER BY  Plant "
        Else
            SqlPass = "SELECT Plant ,    OpeningStock ,     LASTPACKING AS LastMonthPacking	,LASTDISPATCH  AS LastMonthDispatch,	LASTUNIS AS LastMonthUNIS,TillPacking AS MTDPacking , TodayPacking , TillDispatch AS MTDDispatch,  TodayDispatch ,MonDispVal AS [Month Dispatch Value],TdyDispVal AS [Today Dispatch Value] ,TillUnis AS MTDUnis, TodayUnis ,YP AS '" & R & " - Packing'  ,YD AS '" & R & " - Dispatch'   ,  Balance FROM SHP..JCT_OPS_PRODUCTION_DISPATCH_TABLE_PG  WHERE   Plant IN ( 'TAFFETA', 'YARN', 'GARMENTS', 'SYNTHETIC', 'COTTON','[TOTAL]' ) ORDER BY  Plant "
        End If
        PassString(SqlPass)
    End Sub
