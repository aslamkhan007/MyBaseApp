...
Protected Sub LnkReject_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LnkReject.Click
        Dim tr As SqlTransaction
        Dim cn As New Connection

        Dim strVal As String = lblQuotationNo.Text
        Dim Action As String = "Rejection of Quotation No. " + lblQuotationNo.Text
        Try
            ...
            For Each row As GridViewRow In grdDispatchDetail.Rows
                ...
                If (chk = True) Then
                    ...
                    cmd.CommandType = CommandType.StoredProcedure
                    cmd.Parameters.Add("@Quotation_No", SqlDbType.VarChar, 16).Value = row.Cells(0).Text
                    cmd.Parameters.Add("@User_Code", SqlDbType.VarChar, 16).Value = Session("Empcode").ToString
                    cmd.Parameters.Add("@Auth_Type", SqlDbType.VarChar, 20).Value = "PPC"

                    strRemarks = CType(row.FindControl("txtGreighRemark"), TextBox).Text
                    cmd.Parameters.Add("@Remarks", SqlDbType.VarChar, 16).Value = strRemarks



                    cmd.ExecuteNonQuery()
Else
                    ClientScript.RegisterStartupScript(Me.[GetType](), "myalert", "alert('Please Select CheckBox ');", True)
                    Exit Sub
            ...
            EmailReject(strVal, Action, strRemarks)
            bindgrid()
            bindDispatch(lblQuotationNo.Text)
            GreighDateBind(lblQuotationNo.Text)
...
Private Sub EmailReject(ByVal Quotation As String, ByVal Subj As String, ByVal Remark As String)
        Dim sm As New SendMail
        Dim str, body_to, subject As String
        Dim Qtname As String
        ...
        Try
            ...
            'recipients = "hiren@jctltd.com;"

            cc = cc & "; hiren@jctltd.com;"
            bcc = "hitesh@jctltd.com,hiren@jctltd.com, sandeepr@jctltd.com"
