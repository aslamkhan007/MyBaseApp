...
Partial Class OPS_Quotation_Cost_Memo
    ...
    Protected Sub bind_data()
        ...
        Dim memo_dt As String = Request.QueryString("memo_dt")
        Dim item_code As String = Request.QueryString("item_code")
        ...
        Dim sqlstr As String = "jct_costing_data_fetch '" & sort_no & "','" & item_code & "'," & memo_no & ",'" & memo_dt & "'"
        ...
        Try
            ...
            If dr.HasRows Then
                ...
                lblMemoDt.Text = dr("sys_date").ToString
                ...
                lblRmk.Text = dr("rmk").ToString
                lblRmk1.Text = dr("rmk1").ToString
    ...
    Protected Sub Calculate_Finish_Charge()
        ...
        Else
            ...
            lblFinishChrgLight.Text = Math.Round(Val(lblFinishCharge.Text) + fin_mc_cost, 2)
            lblFinishChrgMed.Text = Math.Round(Val(lblFinishCharge.Text) + fin_mc_cost, 2)
            lblFinishChrgDark.Text = Math.Round(Val(lblFinishCharge.Text) + fin_mc_cost, 2)
            lblFinishChrgED.Text = Math.Round(Val(lblFinishCharge.Text) + fin_mc_cost, 2)
    ...
    Protected Sub Calculate_Printing_Charge()
        ...
        Dim Diff As Double
        Diff = Val(lblPrintingCharge.Text) - Val(lblDyesChemFinishingCost.Text)
        Dim impactmtr As Double
        impactmtr = Diff * Val(lblYarnWtMtr.Text)
        Dim TotalPerMtr As Double
        ...
        ShrCost = (Val(ViewState("PrintingCharge")) / (100 - ShrPer)) * 100
        ViewState.Add("Shr_Cost", (ShrCost - ViewState("PrintingCharge")))
        Val_Loss = Val(ShrCost) / Val(lblValRecovery.Text)
        ViewState.Add(("Val_Loss"), (Val_Loss - ShrCost))
        Sell_Exp = Val(Val_Loss) / (100 - Val(lblSellingExpPerc.Text)) * 100
        ViewState.Add(("Sell_Exp"), (Val_Loss - ShrCost))

        Dim impact As Double = Val(lblPrintingCharge.Text) - Val(ViewState("PrintingCharge"))
