...
Partial Class OPS_Quotation_Dispatch_Sch
    ...
    Protected Sub ibtSave_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles ibtSave.Click
        ...
        Else
            ...
            cmd.Parameters.Add("@Line_Item_No", SqlDbType.SmallInt)
            ...
            Try
                ...
                For Each row As GridViewRow In grdDispatchDetail.Rows
                    ...
                    cmd.Parameters("@Line_Item_No").Value = ln
    ...
    Protected Sub cmdPickSchedule_Click(sender As Object, e As System.EventArgs) Handles cmdPickSchedule.Click
        ...
        ' Added by Ashish Sharma on 17 Dec 2014
        'Dim user_code As String = Session("EmpCode")
        'Dim checkusercode As String = user_code.Replace("-", ",")

        Dim Sql As String
        'Sql = "SELECT Sales_Person_Code FROM  jct_ops_quotation_hdr WHERE Quotation_No='" & lblQuotationNo.Text & "' and  Sales_Person_Code= '" & checkusercode & "'"
        ...
        '    ofn.Alert("You Are Not Authorized to Pick and Save Advised Schedule")
        '    Exit Sub
        'End If
        ' Cod ends here
        ...
        sql = "jct_ops_quote_pick_advised_dispatch_sch"
        ...
        Try
            ...
            For Each row As GridViewRow In grdDispatchDetail.Rows
                ...
                cmd = New SqlCommand(sql, con.Connection, tran)
                cmd.CommandType = CommandType.StoredProcedure
    ...
    Protected Sub lnkForwardToPPC_Click(sender As Object, e As System.EventArgs) Handles lnkForwardToPPC.Click
        ...
        ' Code added by ashish sharma on 17 Dec 2014
        'Dim user_code As String = Session("EmpCode")
        'Dim checkusercode As String = user_code.Replace("-", ",")
        ...
        '    ofn.Alert("You Are Not Authorized to Forward consult Team Leader")
        '    Exit Sub
        'End If
        ' Code ends ehre
 ...
 Dim baseurl As String = Request.Url.Scheme + "://" + Request.Url.Authority + Request.ApplicationPath.TrimEnd("/"c) + "/"
        If grdDispatchDetail.Rows.Count = 0 Then
            lblMessage.Text = "Please generate dispatch schedule and save it before Requesting Approval."
            Exit Sub
        End If

        sql = "jct_ops_forward_quot_dispatch_sch"
        Dim con As New Connection
        Dim tran As SqlTransaction
        tran = con.Connection.BeginTransaction()
        Try
            ...
            lblMessage.Text = "Quotation No. " & lblQuotationNo.Text & " has been forwarded to PPC for Approval/Advise.<br/>"
            ScriptManager.RegisterStartupScript(Me, [GetType](), "showalert", "alert(Quotation has been forwarded to PPC for Approval/Advise');", True)
            ...
            Try
                ...
                Dim subject As String = "Required Approval/Advise regarding Quotation No. " + lblQuotationNo.Text + " by " + sender_name
               ' Dim body As String = "Please provide advise or approve Quotation # <a href = 'http://misdev/fusionapps/ops/Quotation_Dispatch_Sch.aspx?quot=" & lblQuotationNo.Text & "'> " & lblQuotationNo.Text & "</a> <br/> " & sender_name & " <br/> Click on Quotation Number to view details."
			     Dim body As String = "Please provide advise or approve Quotation # <a href = '" + baseurl + "'> " & lblQuotationNo.Text & "</a> <br/> " & sender_name & " <br/> Click on Quotation Number to view details."

                Dim bcc As String = "rbaksshi@jctltd.com; hitesh@jctltd.com; harendra@jctltd.com;hiren@jctltd.com;sandeepr@jctltd.com"
                sm.SendMail2(recipients, m_sender, bcc, "noreply@jctltd.com", subject, body)

            Catch ex As Exception
                lblMessage.Text += lblMessage.Text + "<br/><br/>Error Occurred while sending E-Mail to the concerned person(s)."
            End Try

        Catch ex As Exception
            tran.Rollback()
            lblMessage.Text = "Error occurred while forwarding request to concerned for Dispatch Approval/Advise. <br/>" & ex.Message
        End Try

    End Sub

    Protected Sub lnkForwardToDev_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkForwardToDev.Click
