...
Partial Class OPS_RaiseSanctionNote
    ...
    Protected Sub ddlarea_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlarea.SelectedIndexChanged
        ...
        Else
            ...
            qry = "Select ParamCode ,ParmDesc,isnull(MultiValues,'')+'-'+isnull(ProcName,'') as Val,Postback,Reqd_Flag,ISNULL(IsValue,'N') IsValue FROM Jct_Ops_SanctionNote_Parameters where status='A' and AreaCode=" & ddlarea.SelectedItem.Value & " order by ParamCode "
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            qry = "Select 0 as [AreaCode],'--Select--' as [AreaName] Union SELECT AreaCode,AreaName FROM Jct_Ops_SanctioNote_Area_Master WHERE STATUS='A' and parentarea=1015 and  AreaCode IN (1011,1010,1012,1009,1059,1022,1060) ORDER BY AreaName"
    ...
    Protected Sub cmdApply_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdApply.Click
        ...
        'Sandeep 26 Feb 2016
        Dim QtyValue As TextBox
        Dim SampleCostValue As DropDownList
        Dim SampleOrNot As DropDownList
        Dim CostValue As String
        CostValue = ""

        Dim QtySample As Integer
        Dim SampleOrNotValue As String
        SampleOrNotValue = ""
        ...
        With grdParameters
            For i = 0 To .Rows.Count - 1
                ...
                If ParmCode = 257 Then
                    QtyValue = CType(.Rows(i).FindControl("txtValue"), TextBox)
                    QtySample = QtyValue.Text


                End If
                If ParmCode = 258 Then
                    SampleOrNot = CType(.Rows(i).FindControl("ddlValueList"), DropDownList)
                    SampleOrNotValue = SampleOrNot.SelectedItem.Text


                End If
                If ParmCode = 267 Then
                    SampleCostValue = CType(.Rows(i).FindControl("ddlValueList"), DropDownList)
                    CostValue = SampleCostValue.SelectedItem.Text
                End If
        ...
        If QtySample > 2 And CostValue = "Free of Cost" And (SampleOrNotValue = "Sample" Or SampleOrNotValue = "Testing Purpose") Then


            objFun.Alert("Select Proper Sample Cost Paid By !!")

            Exit Sub
        End If


        If QtySample > 2 And (SampleOrNotValue = "Sample" Or SampleOrNotValue = "Testing Purpose") And CostValue = "Free of Cost" Then


            objFun.Alert("Select Paid By Customer !!")
        ...
        Try
            ...
            qry = "JCT_OPS_SANCTIONNOTE_VAL_CHECK '" & SanctionID & "' ,'" & ddlarea.SelectedItem.Value & "'"
            objFun.InsertRecord(qry, Tran, obj.Connection)

            'Try
            '    qry = "JCT_OPS_SANCTIONNOTE_VAL_CHECK"
            '    cmd = New SqlCommand(qry, obj.Connection())
            '    cmd.CommandType = CommandType.StoredProcedure
            '    cmd.Transaction = Tran
            '    cmd.Parameters.Add("@SanctionnoteId", SqlDbType.VarChar).Value = SanctionID
            '    cmd.Parameters.Add("@Area", SqlDbType.Int).Value = ddlarea.SelectedItem.Value
        ...
        Try
            ...
            '    NotifyEmailGroup=
            'End If
            GenrateMail(body, Body1, Body2, Body3, SanctionID, Genratedby_Email, Genratedby_Email, NotifyEmailGroup, "hitesh@jctltd.com,harendra@jctltd.com,rbaksshi@jctltd.com,sandeepr@jctltd.com", "Your SanctionNote No :-" & SanctionID & " has been genrated ")
    ...
    Protected Sub grdParameters_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdParameters.RowDataBound

        If e.Row.RowType = DataControlRowType.DataRow Then
            Dim HdnFld As String = CType(e.Row.FindControl("HiddenField1"), HiddenField).Value
            Dim HdnFldIsValue As String = CType(e.Row.FindControl("HdnIsValue"), HiddenField).Value
            ...
            If LCase(e.Row.Cells(5).Text) = "y" Then
                ...
                If Trim(HdnFldIsValue) = "Y" Or Trim(HdnFldIsValue) = "y" Then Validator_Extnd.Enabled = True
            ...
            If Trim(HdnFld) <> "-" Then
                Try
                    ...
                    If ddlValue.AutoPostBack = True Then
                        '20 JULY 2016
                        '  If Trim(Session("Empcode")) = "R-03613" Then
                        '  qry = "SELECT  'J-01854' AS EmpCode ,'JAIDEEP ANANT JAMKHEDKAR' AS Empname ,  '0'AS AuthLevel UNION SELECT  UPPER(a.EmpCode) AS EmpCode ,c.Empname ,a.AuthLevel FROM Jct_Ops_SanctionNote_Area_Reason_Hiearchy a ,Jct_Ops_SanctioNote_Area_Master b ,dbo.JCT_EmpMast_Base C WHERE   a.AreaCode = b.AreaCode AND a.STATUS = 'A' AND a.STATUS = b.STATUS AND c.empcode = a.empcode AND a.AreaCode = '" & ddlarea.SelectedItem.Value & "' AND a.plant = '" & ddlplant.SelectedItem.Text & "' AND ReasonCode='" & ddlValue.Items(0).Value & "' and a.eff_to is null ORDER BY AuthLevel"
                        '  Else
        ...
        End If
    End Sub
    Private Sub GenrateMail(ByVal Body As String, ByVal Body1 As String, ByVal Body2 As String, ByVal Body3 As String, ByVal OrderNo As String, ByVal SalesPerson_Email As String, ByVal [to] As String, ByVal cc As String, ByVal bcc As String, ByVal Subject As String)
        ...
        strAuth = String.Empty
        Dim cmd3 As SqlCommand
        Dim dr3 As SqlDataReader

        ' strSql = "  SELECT PendingAt,AuthFlag FROM Jct_Ops_SanctionNote_HDR  WHERE SanctionNoteID =  '" + SanctionNote + "'  AND AuthFlag='A' AND PendingAt='' "
        ' strSql = " SELECT  a.EMPCODE ,USERLEVEL FROM JCT_OPS_SanctionNote_AUTHORIZATION_LISTING a JOIN dbo.Jct_Ops_SanctionNote_HDR H ON a.ID=H.SanctionNoteID WHERE ID = '" + OrderNo + "' AND AUTH_DATETIME IS NULL AND PendingAt=a.USERLEVEL ORDER BY USERLEVEL DESC"
        strSql = " SELECT * FROM (SELECT  a.EMPCODE ,USERLEVEL ,( SELECT    MAX(USERLEVEL) AS MaxLevel FROM JCT_OPS_SanctionNote_AUTHORIZATION_LISTING  WHERE ID = A.ID ) AS MaxLevel FROM JCT_OPS_SanctionNote_AUTHORIZATION_LISTING a JOIN dbo.Jct_Ops_SanctionNote_HDR H ON a.ID = H.SanctionNoteID WHERE ID = '" + OrderNo + "' AND AUTH_DATETIME IS NULL AND PendingAt = a.USERLEVEL) Final WHERE MaxLevel=USERLEVEL"


        cmd3 = New SqlCommand(strSql, obj3.Connection())
        dr3 = cmd3.ExecuteReader()
        If (dr3.HasRows) Then

            While (dr3.Read())

                strPending = dr3("EMPCODE").ToString
                strAuth = dr3("EMPCODE").ToString()
        ...
        If strAuth = "P-03062" Then
            [to] = [to].Replace(",pavanmathur@jctltd.com", "")
            [to] = "pavanmathur@jctltd.com" & "," & [to]
    ...
    Protected Sub imgRemoveItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles imgRemoveItem.Click
    ...
    Protected Sub cmdRetreive_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdRetreive.Click
    ...
    Protected Sub grdParameters_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdParameters.SelectedIndexChanged

    End Sub

    Protected Sub ddlValueList_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
    ...
    Protected Sub Button3_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        Dim sqlstr As String = "    SELECT  'Attachments' AS Attachment , ImgName AS AttachedFile FROM  Jct_Ops_SanctionNote_Attachments  WHERE   SanctionNoteID = '" + SanctionID + "'"
    ...
    Protected Sub dtlAttachment_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataListCommandEventArgs) Handles dtlAttachment.ItemCommand
        ...
        If e.CommandName = "Download" Then
            filepath = Server.MapPath("~\\Ops\\Upload\\" + e.CommandArgument.ToString())
        End If
