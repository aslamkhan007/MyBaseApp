...
Partial Class OPS_AreaForSanctionNote
    ...
    Protected Sub cmdApply_Click(sender As Object, e As System.EventArgs) Handles cmdApply.Click
        ...
        Try
            ...
            cmd.ExecuteNonQuery()
            If (lstSortedEmployees.Items.Count > 0) Then
                For i As Int16 = 0 To lstSortedEmployees.Items.Count - 1
                    qry = "Insert into Jct_Ops_SanctioNote_Area_Emp_Auth_Listing(UserCode ,AreaCode ,EmpCode ,UserLevel ,STATUS ,Eff_From ,Eff_To ,CreatedOnHost) values('" & Session("empcode") & "'," & ID & ",'" & lstSortedEmployees.Items(i).Value & "'," & i + 1 & ",'A',getdate(),'12/31/2020','" & Request.ServerVariables("REMOTE_ADDR") & "' )"
                    cmd = New SqlCommand(qry, obj.Connection)
                    cmd.Transaction = tran
                    cmd.ExecuteNonQuery()
                Next
