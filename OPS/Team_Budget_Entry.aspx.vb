...
Partial Class Team_Budget_Entry
    ...
    Protected Sub imb_fetch_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imb_fetch.Click
        ...
        Me.GridView2.Visible = False
        ''-----ReFill GridView1
        sqlpass = "exec jct_ops_sales_order_fetch_New '" & LTrim(RTrim(Me.ddl_action.Text)) & "'," & _
                    Right(RTrim(Me.ddl_yearmonth.Text), 6) & ",'" & _
                    LTrim(RTrim(Me.ddl_team.Text)) & "','" & _
                    LTrim(RTrim(Me.txt_orderno.Text)) & "' ,'" & Session("CompanyCode") & "' "
        ...
        Try
            ...
            GridView1.DataBind()
            sql = "Select * from jct_ops_monthly_planning where mode='Freezed' and yearmonth=" + ddl_yearmonth.SelectedItem.Text + " and Location='" + ddlPlant.SelectedItem.Text + "'"
            If (obj1.FetchValue(sql)) Then
                If (GridView1.Columns(11).Visible = False) Then
  ...
  LTrim(RTrim(Me.txt_orderno.Text)) & "' ,'" & Session("CompanyCode") & "' "
    ...
    Protected Sub lnk_apply_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnk_apply.Click
        ...
        If UCase(LTrim(RTrim(Me.ddl_action.Text))) = "ADD" Then
            ...
            Try
                ...
                'FMsg.CssClass = "addmsg"
                'FMsg.Display()
                Dim script1 As String = "alert('Record Saved Successfully.!!');"
        ...
        If UCase(LTrim(RTrim(Me.ddl_action.Text))) = "MODIFY" Or UCase(LTrim(RTrim(Me.ddl_action.Text))) = "SHORT CLOSE" Or UCase(LTrim(RTrim(Me.ddl_action.Text))) = "AUTHORIZE" Then
            ...
            Catch ex As Exception
                ...
                'FMsg.CssClass = "addmsg"
                'FMsg.Display()
                Dim script1 As String = "alert('Error Occured -  " + ex.Message + " ');"
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "ServerControlScript", script1, True)
                Me.lnk_exit_Click(sender, e)       ''  Execute EXIT button script
                Exit Sub
    ...
    Protected Sub PopulatePieChart(ByVal sql As String, ByRef chart As Chart)
        Try
            ...
            chart.Titles(0).Text = "Team Loom Status"
            ...
            series1.XValueMember = ds1.Tables(0).Columns(0).Caption
            series1.YValueMembers = ds1.Tables(0).Columns(1).Caption
    ...
    Protected Sub PopulateBarChart(ByVal sql As String, ByRef chart As Chart)
        Try
            ...
            chart.Series("Series1").XValueMember = ds1.Tables(0).Columns(0).Caption
            chart.Series("Series1").YValueMembers = ds1.Tables(0).Columns(1).Caption
            chart.Series("Series2").XValueMember = ds1.Tables(0).Columns(0).Caption
            chart.Series("Series2").YValueMembers = ds1.Tables(0).Columns(2).Caption
            chart.Series("Series3").XValueMember = ds1.Tables(0).Columns(0).Caption
            chart.Series("Series3").YValueMembers = ds1.Tables(0).Columns(3).Caption
            chart.DataBind()
        Catch ex As Exception
            MsgBox("Exception")
        Finally
    ...
    Protected Sub lnk_teamlist_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnk_teamlist.Click
        ...
        Try
            obj1.FillGrid(sqlpass, GridView2)
            sql = "SELECT 'Looms Booked : ' + CONVERT(VARCHAR(10), sel_qty) AS [Looms Booked],sel_qty AS [Looms Consumed] FROM jct_ops_team_budget_check_list_temp where userid='" + Session("EmpCode") + "' UNION SELECT 'Looms Available : ' + CONVERT(VARCHAR(10), ( norm_value - sel_qty )) AS [Looms Available], (norm_value - sel_qty ) AS [Rem Looms] FROM dbo.jct_ops_team_budget_check_list_temp where userid='" + Session("EmpCode") + "'"
            PopulatePieChart(sql, Chart1)
        Catch ex As Exception
            obj.closecn()
            FMsg.Message = (ex.Message)
            FMsg.CssClass = "addmsg"
    ...
    Protected Sub lnk_blendlist_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnk_blendlist.Click
        ...
        Try
            ...
            GridView2.DataBind()
            sql = "SELECT blend,norm_value,sum(sel_qty) as sel_qty,(norm_value-Sum(sel_qty)) AS Balance_Qty FROM  dbo.jct_ops_team_budget_check_list_temp WHERE userid='" + Session("EmpCode") + "' GROUP BY blend ,norm_value "

            PopulateBarChart(sql, Chart2)
        Catch ex As Exception
            obj.closecn()
            FMsg.Message = (ex.Message)
            FMsg.CssClass = "addmsg"
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        'sql = "Select * from jct_ops_monthly_planning where mode='Freezed' and yearmonth=" + ddl_yearmonth.SelectedItem.Text + " "
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If (Meters.Text = 0.0) Then
                Meters.Text = e.Row.Cells(9).Text
            End If
            'e.Row.Cells(8).Visible = False
    ...
    Protected Sub lnk_update_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        For i = 0 To Me.GridView1.Rows.Count - 1
            ...
            If CType(GridView1.Rows(i).FindControl("lnk_update"), LinkButton).ClientID.Equals(lnk.ClientID) Then
                ...
                sql = "Select * from jct_ops_monthly_planning where yearmonth=" + ddl_yearmonth.SelectedItem.Text + " and mode='Freezed'  "
                If (obj1.CheckRecordExistInTransaction(sql)) Then
                    If (reason.SelectedItem.Text <> "") Then
                        'selqty = 0.1 * selqty + selqty
                        CType(GridView1.Rows(i).FindControl("txt_selqty"), TextBox).Text = selqty
    ...
    Protected Sub lnkfetchOrders_Click(sender As Object, e As System.EventArgs) Handles lnkfetchOrders.Click
        Dim obj3 As Connection = New Connection()
        ' sql = " SELECT * FROM JanWeavePlan_FromExcelSheetofPlanning_RemainingOrders"
        '  sql = " SELECT * FROM jatin_weaveplan"
        sql = "JCT_OPS_FETCH_SALEORDER_IN_PLANNING"
        ...
        Try
            If (dr.HasRows) Then
                ...
                While (dr.Read())
                    ...
                    sqlpass = "jct_ops_sales_team_budget_entry"
                    Dim sqlcmd As SqlCommand = New SqlCommand(sqlpass, obj3.Connection())
                    sqlcmd.CommandType = CommandType.StoredProcedure
                    sqlcmd.CommandTimeout = 500
                    sqlcmd.Parameters.Add("@action", SqlDbType.VarChar, 20).Value = "Add"
                    sqlcmd.Parameters.Add("@YearMonth", SqlDbType.Decimal).Value = 201302
                    ...
                    obj3.ConClose()
                    Dim script1 As String = "alert('Sale Orders Added ..!!');"
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "ServerControlScript", script1, True)
                End While
            End If
        Catch ex As Exception
