            ...
            If Val(CType(GridView1.Rows(i).FindControl("txt_move_qty"), TextBox).Text) > 0 Then
                ...
                If (UCase(LTrim(RTrim(Me.ddl_action.Text))) = "ADD" Or UCase(LTrim(RTrim(Me.ddl_action.Text))) = "MODIFY") Then
                    ...
                    If dr.HasRows = True Then
                        ...
                        'Dim mvq As Decimal = Val(GridView1.Rows(i).Cells(9).Text)
                        'Dim rvq As Decimal = Val(GridView1.Rows(i).Cells(8).Text)

                        If (Val(dr.Item(0)) + Val(CType(GridView1.Rows(i).FindControl("txt_move_qty"), TextBox).Text)) > Val(GridView1.Rows(i).Cells(8).Text) Then
                            FMsg.Message = "Prev. " & dr.Item(0) & " Reqd.Qty. " & CType(GridView1.Rows(i).FindControl("txt_move_qty"), TextBox).Text & "  More than Rcvd. Qty. of " & LTrim(RTrim(GridView1.Rows(i).Cells(0).Text)) + " - " + LTrim(RTrim(GridView1.Rows(i).Cells(2).Text))
                            FMsg.CssClass = "errormsg"
                            FMsg.Display()
                            dr.Close()
                            obj.closecn()
