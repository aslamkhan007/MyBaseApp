...
Partial Class frm_pending_orders
    ...
    Protected Sub cmdFetch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdFetch.Click
        ...
        'sqlpass = "exec jct_pp_pending_sales_orders "
    ...
    Public Sub saleperson()
        'Dim SqlPass As String = "select distinct b.group_desc   from  miserp.som.dbo.jct_team_saleperson_mapping  a, miserp.som.dbo.m_cust_group b where a.sale_person_code  = b.group_no and   b.group_type  ='SALESP' and   b.status  = 'O'  and (team_code= '" & ddlteam.SelectedValue & "' or '" & ddlteam.SelectedValue & "'='ALL') order by b.group_desc  "
        Dim SqlPass As String = "SELECT DISTINCT group_desc  from jct_pp_pending_so where (team_code= '" & ddlteam.SelectedValue & "' or '" & ddlteam.SelectedValue & "'='ALL') order by group_desc   "
        Dim Dr As SqlDataReader = obj.FetchReader(SqlPass)
        Try
            If Dr.HasRows = True Then
    ...
    Public Sub customer()
        Dim SqlPass As String = "select distinct cust_name from  jct_pp_pending_so  where (team_code= '" & ddlteam.SelectedValue & "' or '" & ddlteam.SelectedValue & "'='ALL') and (group_desc= '" & ddlsaleperson.SelectedValue & "' or '" & ddlsaleperson.SelectedValue & "'='ALL') and (sale_catg= '" & ddlcatg.SelectedValue & "' or '" & ddlcatg.SelectedValue & "'='ALL') order by cust_name  "
    ...
    Public Sub catg()
        Dim SqlPass As String = "select distinct sale_catg from  jct_pp_pending_so  where (team_code= '" & ddlteam.SelectedValue & "' or '" & ddlteam.SelectedValue & "'='ALL') and (group_desc= '" & ddlsaleperson.SelectedValue & "' or '" & ddlsaleperson.SelectedValue & "'='ALL') order by sale_catg  "
        Dim Dr As SqlDataReader = obj.FetchReader(SqlPass)
        Try
            If Dr.HasRows = True Then
                Me.ddlcatg.Items.Clear()
                ddlcatg.Items.Add("ALL")
                While Dr.Read()
                    If Not (Dr.Item(0) Is System.DBNull.Value) Then
                        Me.ddlcatg.Items.Add(Trim(Dr.Item(0)))
                    End If
                End While
            Else
            End If
    ...
    Public Sub BindData1()
        Dim Sqlpass As String
        Sqlpass = "SELECT order_no 'Order No',convert(varchar(11),order_dt,101) 'order Dt',convert(varchar(11),req_dt,101) 'Need Dt',cust_name 'Customer',team_code 'Team',group_desc 'Sales Person',sale_catg 'Category',item_no 'Sort No',variant 'Variant', balance_qty as 'Pending Qty' FROM  jct_pp_pending_so where (team_code = '" & ddlteam.Text & "' or '" & ddlteam.Text & "' = 'ALL') and (group_desc = '" & ddlsaleperson.Text & "' or '" & ddlsaleperson.Text & "' = 'ALL') and (sale_catg = '" & ddlcatg.Text & "' or '" & ddlcatg.Text & "' = 'ALL') and (cust_name = '" & ddlcustomer.Text & "' or '" & ddlcustomer.Text & "' = 'ALL') order by team_code,group_desc "
        CstModule.FillGrid(Sqlpass, grdGrid1)
    End Sub
    Protected Sub cmdexcel1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdexcel1.Click
        'Try
        GridViewExportUtil.Export("pendorders.xls", Me.grdGrid1)
        'Catch ex As Exception
    ...
    Protected Sub ddlsaleperson_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlsaleperson.SelectedIndexChanged
        catg()
    End Sub
