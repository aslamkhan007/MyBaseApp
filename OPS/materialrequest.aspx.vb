...
Check:  Next
    ...
    Protected Sub lnkSave_Click(sender As Object, e As System.EventArgs) Handles lnkSave.Click
        ...
        If (ddlRequestType.Text = "Material Return") Then
            Dim validgr As Boolean
            validgr = True
            For i As Integer = 0 To GridView2.Rows.Count - 1
                Dim GrNumber As TextBox = DirectCast(GridView2.Rows(i).FindControl("txtGrNo"), TextBox)
                If GrNumber.Text.Length < 6 Then
                    If ((GrNumber.Text) = "0" Or (GrNumber.Text) = "00" Or (GrNumber.Text) = "000") Then
                        validgr = False
                    End If
                    validgr = False
            ...
            If validgr = False Then
                Dim script As String = "alert('Enter Valid GrNo..!!');"
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "ServerControlScript", script, True)
                Exit Sub
            ...
            If (flag = 1) Then
                ...
                If (ViewState("RefreshFlag") = 1) Then
                    ...
                    For i As Integer = 0 To GridView2.Rows.Count - 1
                        ...
                        Dim OrderNo As String = DirectCast(GridView2.Rows(i).FindControl("lblOrder"), Label).Text
                        Dim Line_no As String = DirectCast(GridView2.Rows(i).FindControl("lblLineNo"), Label).Text
                        Dim Shade As String = DirectCast(GridView2.Rows(i).FindControl("lblShade"), Label).Text
                        ...
                        Try
                            ...
                            If (eReasonslist <> "") Then
                                ...
                                Cmd.Parameters.Add("@Item_no", SqlDbType.VarChar, 200).Value = item_no
                                Cmd.Parameters.Add("@Invoice_no", SqlDbType.VarChar, 30).Value = invoice_no
                                Cmd.Parameters.Add("@Inv_date", SqlDbType.DateTime, 20).Value = inv_date
                                Cmd.Parameters.Add("@OrderNo", SqlDbType.VarChar, 100).Value = OrderNo
                                Cmd.Parameters.Add("@Line_no", SqlDbType.VarChar, 100).Value = Line_no
                                Cmd.Parameters.Add("@Shade", SqlDbType.VarChar, 100).Value = Shade
                                Cmd.Parameters.Add("@Cust_name", SqlDbType.VarChar, 100).Value = cust_name
                                Cmd.Parameters.Add("@SalePerson", SqlDbType.VarChar, 100).Value = ddlSalesPerson.SelectedItem.Value
                                Cmd.Parameters.Add("@EmpCode", SqlDbType.VarChar, 10).Value = Session("EmpCode")
                                Cmd.Parameters.Add("@Instruction", SqlDbType.VarChar, 200).Value = txtinstructions.Text
    ...
    Private Sub SendMail()
        ...
        Dim sb As New StringBuilder()
        Dim email1, email2,email3 As String
        ...
        End If
'23 MARCH 2016
        Sql = "SELECT DISTINCT A.E_MailID FROM MISTEL A JOIN jct_ops_material_request M ON REPLACE(A.empcode,'-','')=M.sales_person  WHERE RequestID = '" + ViewState("RequestID") + "' "
        If (ObjFun.CheckRecordExistInTransaction(Sql)) Then

            email3 = ObjFun.FetchValue(Sql)

        Else

            email3 = "hitesh@jctltd.com"
        ...
        sb.AppendLine("<tr><th> Invoice No</th> <th> Sort</th> <th> OrderNo</th><th> LineNo</th><th> Shade</th><th> Customer</th> <th> Sale Person</th> <th> Invoice Qty</th><th> Invoice Date </th> <th> Return Qty</th>  <th> Auth. Pending At</th> <th> Freight Paid By</th>  </tr>")
        'Sql = "SELECT isnull(a.invoice_no,'') as invoice_no,isnull(a.item_no,'') as item_no,isnull(a.customer,'') as customer,isnull(b.empname,'') as sales_person,isnull(Convert(numeric(12,2),a.invoice_qty),0) as invoice_qty,isnull(Convert(Numeric(12,2),a.ret_qty),0) as ret_qty ,isnull(a.FlagAuth,'') as FlagAuth FROM dbo.jct_ops_material_request a INNER JOIN dbo.JCT_EmpMast_Base b ON a.sales_person=REPLACE(b.empcode,'-','') WHERE a.RequestID=" + ViewState("RequestID")
        Sql = "JCT_OPS_SANCTION_PENDING_AT_MATERIAL_RETURN"
        Dim cmd As SqlCommand = New SqlCommand(Sql, Obj.Connection())
        cmd.CommandType = CommandType.StoredProcedure
        ...
        If (Dr.HasRows) Then
            While (Dr.Read())
                ...
                If (Dr(6).ToString = "" Or Dr(6).ToString() = "CEO") Then
                    ViewState("PendingAt") = Dr(6).ToString()
                    sb.AppendLine("<tr> <td> " & Dr(0).ToString & " </td> <td> " & Dr(1).ToString & "  </td> <td> " & Dr("OrderNo").ToString & "</td>  <td> " & Dr("Line_no").ToString & "</td>  <td> " & Dr("Shade").ToString & "</td> <td> " & Dr(2).ToString & "</td>  <td>" & Dr(3).ToString & " </td>  <td>" & Dr(4).ToString & " </td> <td> " & Dr(8).ToString() & " </td><td> CEO </td><td>" & Dr("FreightPaidBy").ToString & " </tr> ")
                End If
                Sql = "Select empname from jct_empmast_base where active='Y' and  empcode='" + Dr(6).ToString().Split(",")(0) + "'"
                Dim empname As String = ""
                Dim obj2 As Connection = New Connection
                ...
                dr1.Close()
                obj2.ConClose()
                sb.AppendLine("<tr> <td> " & Dr(0).ToString & " </td> <td> " & Dr(1).ToString & "  </td>  <td> " & Dr("OrderNo").ToString & "</td>  <td> " & Dr("Line_no").ToString & "</td>  <td> " & Dr("Shade").ToString & "</td>  <td> " & Dr(2).ToString & "</td>  <td>" & Dr(3).ToString & " </td>  <td>" & Dr(4).ToString & " </td>  <td> " & Dr(8).ToString() & " </td> <td>" & Dr(5).ToString & "</td>  <td>" & empname & "</td> <td>" & Dr("FreightPaidBy").ToString & " </td> </tr> ")
            End While
        ...
        If (ViewState("PendingAt") = "") Then
           [to] = email1 + "," + email2 + "," + email3
            ...
            '   body = body + "To " + [to]
        Else
         [to] = "hitesh@jctltd.com," + email2 + "," + email3
            '  body = body + "To " + [to]
        End If
        ...
        Try
            ...
            ElseIf (e.CommandName = "Refresh") Then
                ...
                If ViewState("CurrentTable") IsNot Nothing Then
                    ...
                    Sql = "JCT_OPS_MR_Fetch_Invoice_Detail_New"
                    ...
                    If dr.HasRows Then
                        While dr.Read()
                            ...
                            dtCurrentTable.Rows(rowindex__2)("OrderNo") = dr("OrderNo").ToString()
                            dtCurrentTable.Rows(rowindex__2)("Line_no") = dr("Line_no").ToString()
                            dtCurrentTable.Rows(rowindex__2)("Shade") = dr("Shade").ToString()
                            ...
                            drCurrentRow("Line_no") = String.Empty
    ...
    Private Sub SetInitialRow()
        Try
            ...
            dt.Columns.Add(New DataColumn("Line_no", GetType(String)))
            dt.Columns.Add(New DataColumn("Shade", GetType(String)))
            ...
            If ViewState("CurrentTable") IsNot Nothing Then
                ...
                If dtCurrentTable.Rows.Count > 0 Then
                    For i As Integer = 0 To dtCurrentTable.Rows.Count - 1
                        ...
                        Dim ItemNo As Label = DirectCast(GridView2.Rows(rowIndex).Cells(6).FindControl("lblItemNo"), Label)
                        Dim InvoiceDt As Label = DirectCast(GridView2.Rows(rowIndex).Cells(7).FindControl("lblInvoiceDt"), Label)
                        Dim OrderNo As Label = DirectCast(GridView2.Rows(rowIndex).Cells(8).FindControl("lblOrder"), Label)
                        Dim Line_no As Label = DirectCast(GridView2.Rows(rowIndex).Cells(8).FindControl("lblLineNo"), Label)
                        Dim Shade As Label = DirectCast(GridView2.Rows(rowIndex).Cells(8).FindControl("lblShade"), Label)
                        Dim Customer As Label = DirectCast(GridView2.Rows(rowIndex).Cells(8).FindControl("lblCustomer"), Label)
                        Dim SalesPerson As Label = DirectCast(GridView2.Rows(rowIndex).Cells(9).FindControl("lblSalesPerson"), Label)
                        ...
                        dtCurrentTable.Rows(i)("OrderNo") = Customer.Text
                        dtCurrentTable.Rows(i)("Line_no") = Customer.Text
                        dtCurrentTable.Rows(i)("Shade") = Customer.Text
            ...
            If ViewState("CurrentTable") IsNot Nothing Then
                ...
                If dt.Rows.Count > 0 Then
                    For i As Integer = 0 To dt.Rows.Count - 1
                        ...
                        Line_no.Text = dt.Rows(i)("Line_no").ToString()
