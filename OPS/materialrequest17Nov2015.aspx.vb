...
ObjFun.FillList(ddlActionToBeTaken, Qry)
            ...
            If (Request.QueryString("RequestID") <> "") Then
                ...
                Sql = "SELECT Freight_by,reason,ret_qty,bales,invoice_qty,item_no,invoice_no ,invoice_date,customer,sales_person,Instructions,Enclouser,FlagAuth,sales_person,Plant,DESCRIPTION FROM dbo.jct_ops_material_request WHERE RequestID=" + Request.QueryString("RequestID")
                ...
                If (dr.HasRows) Then
                    ...
                    While (dr.Read())
                        ...
                        Dim qry As String = "SELECT invoice_no as InvoiceNo,Freight_by as Freightby,Convert(varchar,invoice_qty) AS Qty,Convert(varchar,ret_qty) as ReturnQty,Convert(varchar,bales) as Bales,item_no as ItemNo,Convert(varchar,invoice_date) as  InvoiceDt,customer as Customer,sales_person as SalesPerson,reason FROM dbo.jct_ops_material_request WHERE RequestID=" + Request.QueryString("RequestID")
...
Check:  Next
                        ...
                        Try
                            ...
                            If (eReasonslist <> "") Then
                                ...
                                If (Not String.IsNullOrEmpty(txtFreightValue.Text)) Then
                                    If ddlRequestType.SelectedItem.Text = "Material Return" Then txtFreightValue.Text = 0
    ...
    Private Sub SendMail()
        ...
        sb.AppendLine("Details are Shown below : <br/><br/>")
        sb.AppendLine("<table class=gridtable>")
        sb.AppendLine("<tr><th> Invoice No</th> <th> Sort</th> <th> Customer</th> <th> Sale Person</th> <th> Invoice Qty</th><th> Invoice Date </th> <th> Return Qty</th>  <th> Auth. Pending At</th> <th> Freight Paid By</th> <th> Sale Price(Per Unit)</th> <th>Value Involved</th> </tr>")
        'Sql = "SELECT isnull(a.invoice_no,'') as invoice_no,isnull(a.item_no,'') as item_no,isnull(a.customer,'') as customer,isnull(b.empname,'') as sales_person,isnull(Convert(numeric(12,2),a.invoice_qty),0) as invoice_qty,isnull(Convert(Numeric(12,2),a.ret_qty),0) as ret_qty ,isnull(a.FlagAuth,'') as FlagAuth FROM dbo.jct_ops_material_request a INNER JOIN dbo.JCT_EmpMast_Base b ON a.sales_person=REPLACE(b.empcode,'-','') WHERE a.RequestID=" + ViewState("RequestID")
        Sql = "JCT_OPS_SANCTION_PENDING_AT_MATERIAL_RETURN"
        Dim cmd As SqlCommand = New SqlCommand(Sql, Obj.Connection())
        cmd.CommandType = CommandType.StoredProcedure
        ...
        If (Dr.HasRows) Then
            While (Dr.Read())
                ...
                If (Dr(6).ToString = "" Or Dr(6).ToString() = "CEO") Then
                    ViewState("PendingAt") = Dr(6).ToString()
                    sb.AppendLine("<tr> <td> " & Dr(0).ToString & " </td> <td> " & Dr(1).ToString & "  </td>  <td> " & Dr(2).ToString & "</td>  <td>" & Dr(3).ToString & " </td>  <td>" & Dr(4).ToString & " </td> <td> " & Dr(8).ToString() & " </td><td> CEO </td><td>" & Dr("FreightPaidBy").ToString & " </td> <td>" & Dr("SalePrice").ToString & " </td> <td>" & Dr("ValueInvolved").ToString & "</td> </tr> ")
                End If
                Sql = "Select empname from jct_empmast_base where active='Y' and  empcode='" + Dr(6).ToString().Split(",")(0) + "'"
                Dim empname As String = ""
                Dim obj2 As Connection = New Connection
                ...
                dr1.Close()
                obj2.ConClose()
                sb.AppendLine("<tr> <td> " & Dr(0).ToString & " </td> <td> " & Dr(1).ToString & "  </td>  <td> " & Dr(2).ToString & "</td>  <td>" & Dr(3).ToString & " </td>  <td>" & Dr(4).ToString & " </td>  <td> " & Dr(8).ToString() & " </td> <td>" & Dr(5).ToString & "</td>  <td>" & empname & "</td> <td>" & Dr("FreightPaidBy").ToString & " </td><td>" & Dr("SalePrice").ToString & " </td> <td>" & Dr("ValueInvolved").ToString & "</td> </tr> ")
            End While
        ...
        bcc = "harendra@jctltd.com,rbaksshi@jctltd.com,ashish@jctltd.com,manishk@jctltd.com"
        '[to] = ("jatindutta@jctltd.com")
        'Email Address of Receiver
        'cc = "jatindutta@jctltd.com,jagdeep@jctltd.com,hitesh@jctltd.com"
        subject = " Material Return Request - " + ViewState("Customer")
    ...
    Protected Sub chbSelectAll_CheckedChanged(sender As Object, e As System.EventArgs)
    ...
    Protected Sub txtSanctionID_TextChanged(sender As Object, e As System.EventArgs) Handles txtSanctionID.TextChanged
    ...
    Protected Sub chbReasons_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles chbReasons.SelectedIndexChanged
        For i As Integer = 0 To chbReasons.Items.Count - 1
            If chbReasons.Items(i).Selected Then
    ...
    Protected Sub txtReturnQty_TextChanged(sender As Object, e As System.EventArgs)
    ...
    Protected Sub cmdTransport_Click(sender As Object, e As System.EventArgs) Handles cmdTransport.Click
