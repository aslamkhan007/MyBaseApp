...
Partial Class OPS_raw_material_receipt
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            sqlpass = "select distinct ltrim(rtrim(a.vendor_name)) + '|' + ltrim(rtrim(a.vendor_code)) " & _
                    "from miserp.common.dbo.pur_company_vendor_master a " & _
                    "where a.company_code='" & UCase(LTrim(RTrim(Session("companycode")))) & "' " & _
                    "and ltrim(rtrim(a.vendor_code)) not in " & _
                    "(select distinct y.vendor_code from miserp.pomdb.dbo.pur_gi_detail x, " & _
                    "miserp.pomdb.dbo.pur_gi_header y, miserp.common.dbo.ims_stock_master z " & _
                    "where x.gi_no = y.gi_no and x.stock_no = z.stock_no and z.stock_type not in ('0','2','3')) " & _
                    "union " & _
                    "select distinct ltrim(rtrim(a.vendor_name)) + '|' + ltrim(rtrim(a.vendor_code)) " & _
                    "from miserp.common.dbo.pur_company_vendor_master a " & _
                    "where a.company_code='" & UCase(LTrim(RTrim(Session("companycode")))) & "' " & _
                    "and ltrim(rtrim(a.vendor_code)) in " & _
                    "(select distinct y.vendor_code from miserp.pomdb.dbo.pur_gi_detail x, " & _
                    "miserp.pomdb.dbo.pur_gi_header y, miserp.common.dbo.ims_stock_master z " & _
                    "where x.gi_no = y.gi_no and x.stock_no = z.stock_no and z.stock_type in ('0','2','3')) " & _
                    "and ltrim(rtrim(a.vendor_code)) in " & _
                    "(select distinct y.vendor_code from miserp.pomdb.dbo.pur_gi_detail x, " & _
                    "miserp.pomdb.dbo.pur_gi_header y, miserp.common.dbo.ims_stock_master z " & _
                    "where x.gi_no = y.gi_no and x.stock_no = z.stock_no and z.stock_type not in ('0','2','3')) " & _
                    "order by ltrim(rtrim(a.vendor_name)) + '|' + ltrim(rtrim(a.vendor_code)) "
    ...
    Protected Sub ddl_action_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddl_action.SelectedIndexChanged
        ...
        If UCase(LTrim(RTrim(Me.ddl_action.Text))) = "ADD" Then ''And Me.lbt_add.Text = "ADD" Then
            ...
            If Me.lbt_add.Text = "ADD" Then
                ...
                Me.txt_other_value.Enabled = True
        ...
        If UCase(LTrim(RTrim(Me.ddl_action.Text))) = "VIEW" Then
            ...
            Me.txt_other_value.Enabled = False
    ...
    Protected Sub lbt_apply_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbt_apply.Click
        ...
        If LTrim(RTrim(Me.txt_other_value.Text)) = "" Then
            FMsg.Message = "Pl. enter Other Value. "
            FMsg.CssClass = "errormsg"
            FMsg.Display()
            Me.txt_other_value.Focus()
            Exit Sub
        End If
            ...
            Try
                ...
                For i = 0 To Me.ddl_supplier_code.Items.Count - 1
                    ...
                    sqlpass = "insert into jct_ops_raw_material_receipt_header_supplier " & _
                            ...
                            "userid, hostname, company_code, supplier_serial_no, system_date) " & _
                            ...
                            "(select isnull(count(*),0) + 1 from jct_ops_raw_material_receipt_header_supplier " & _
                            "where tran_no = '" & LTrim(RTrim(Me.txt_tranno.Text)) & "'), getdate() "
                ...
                sqlpass = "exec jct_ops_raw_material_receipt_entry '" & _
                            ...
                            LTrim(RTrim(Me.txt_other_value.Text)) & ",'" & _
                            reqddt & "'," & _
    ...
    Protected Sub imb_tran_fetch_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imb_tran_fetch.Click
        ...
        If UCase(LTrim(RTrim(Me.ddl_action.Text))) = "MODIFY" Or UCase(LTrim(RTrim(Me.ddl_action.Text))) = "VIEW" Or UCase(LTrim(RTrim(Me.ddl_action.Text))) = "CANCEL" Or UCase(LTrim(RTrim(Me.ddl_action.Text))) = "SHORT CLOSE" Or UCase(LTrim(RTrim(Me.ddl_action.Text))) = "AUTHORIZE" Then
            ...
            sqlpass = "select a.tran_no, a.location, a.group_code, a.group_desc, a.supplier_code, a.supplier_name, " & _
                    ...
                    "when 'C' then 'CANCEL' when 'S' then 'SHORT CLOSE' end 'status', a.system_date, isnull(a.other_value,0) " & _
                    "from jct_ops_raw_material_receipt_header a " & _
            ...
            If dr.HasRows = True Then
                ...
                trandate = dr.Item(36)
                Me.txt_other_value.Text = dr.Item(37)
            ...
            ''Fill Detail Part
            sqlpass = "select distinct ltrim(rtrim(a.supplier_name)) + '|' + ltrim(rtrim(a.supplier_code)), supplier_serial_no " & _
                    "from jct_ops_raw_material_receipt_header_supplier a " & _
                    "where a.tran_no='" & LTrim(RTrim(Me.txt_tranno.Text)) & "' " & _
                    "and a.company_code='" & UCase(LTrim(RTrim(Session("companycode")))) & "' " & _
                    "order by supplier_serial_no, ltrim(rtrim(a.supplier_name)) + '|' + ltrim(rtrim(a.supplier_code)) "
    ...
    Protected Sub ddl_supplier_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddl_supplier.TextChanged
        ...
        If UCase(LTrim(RTrim(Me.ddl_action.Text))) = "ADD" Or UCase(LTrim(RTrim(Me.ddl_action.Text))) = "MODIFY" Then
            ...
            For i = 0 To Me.ddl_supplier_code.Items.Count - 1
                'If UCase(Mid(LTrim(RTrim(Me.ddl_supplier_code.Items(i).Text)), InStr(Me.ddl_supplier_code.Items(i).Text, "-") + 1, Len(LTrim(RTrim(Me.ddl_supplier_code.Items(i).Text))))) = UCase(LTrim(RTrim(Me.ddl_supplier.Text))) Then
                If UCase(LTrim(RTrim(Me.ddl_supplier_code.Text))) = UCase(LTrim(RTrim(Me.ddl_supplier.Text))) Then
                    exist = "Y"
                    Exit For
                Else
                    exist = "N"
    ...
    Protected Sub txt_pay_term_code_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_pay_term_code.TextChanged
        ...
        If InStr(LTrim(RTrim(Me.txt_pay_term_code.Text)), "-") > 0 Then

            Me.txt_pay_term_code.Text = Mid(LTrim(RTrim(Me.txt_pay_term_code.Text)), 1, InStr(LTrim(RTrim(Me.txt_pay_term_code.Text)), "-") - 2)
        ...
        Else
            ...
            If LTrim(RTrim(Me.txt_pay_term_code.Text)) <> "" Then
                ...
                ''-----Set Pay term according to Supplier in case multiple supplier selected by user
                sqlpass = "select a.pay_term_no, a.pay_term_desc, " & _
                        "isnull(convert(varchar(11),a.effective_date,101),''), isnull(convert(varchar(11),a.expiry_date,101),'') " & _
