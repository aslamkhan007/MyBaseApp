...
Partial Class OPS_soa1
    ...
    Public Sub BindData()
        ...
        Sqlpass = "select cust_no,cust_name,city,doc_no 'invoice_no',convert(varchar(10),doc_date,103) 'Invoice_dt',crdoc_no 'perticular', drcr_reason_descrp 'CrDr-Reason',isnull(convert(varchar(10),crdoc_dt,103),' ') 'perticulat_dt',isnull(inst_no,' ') 'Chk_No',isnull(debit,0.0) 'Debit',isnull(credit,0.0) 'Credit' , isnull(cd,0) 'CD', isnull(short_rec,0) 'ShortRec',isnull(claim,0) 'Claim',isnull(foc,0) 'FOC', isnull(frt_other,0) 'FreightOther',isnull(rate_diff,0) 'Ratediff',isnull(rg,0) 'RG' ,isnull(Convert(varchar,prev_uptodate),'') 'Balance' FROM ops_customer_statement_of_account  order by serial"
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            Totcd = Totcd + Decimal.Parse(e.Row.Cells(12).Text)

        ElseIf e.Row.RowType = DataControlRowType.Footer Then
            e.Row.Cells(0).Text = "Total"
            e.Row.Cells(1).Text = Totcd
        End If
    End Sub
End Class
